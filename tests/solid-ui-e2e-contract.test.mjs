...
const displayMenu = fs.readFileSync(new URL('../src/components/FloatingViewMenu.tsx', import.meta.url), 'utf8')
...
test('isolated SOLID browser workflow has durable language-independent UI anchors', () => {
  ...
  assert.match(viewport, /<FloatingViewMenu styleName=\{visualStyle\}/)
  assert.match(displayMenu, /data-testid="visual-style-picker"/)
  assert.match(displayMenu, /data-visual-style=\{styleName\}/)
