/* Production canvas acceptance: Shell must select an opening face on the
 * WebGL canvas, accept wall thickness and commit a parametric shell feature. */
...
try {
  ...
  await invoke('shell', 'MODIFY')
  await dialog.waitFor({ state: 'visible' })
  await page.waitForFunction(() => window.useApp?.getState?.().shellMode === true)
  ...
  for (const [x, y] of [[.52, .50], [.55, .55], [.47, .55], [.50, .58]]) {
    ...
    selected = await page.waitForFunction(() => (window.useApp?.getState?.().shellPicks?.length ?? 0) === 1, null, { timeout: 900 }).then(() => true).catch(() => false)
    if (selected) break
  }
  assert.ok(selected, 'Shell must accept a canvas opening-face selection')
  await dialog.locator('input[type="number"]').fill('2')
  await dialog.getByRole('button', { name: '确定' }).click()
  await page.waitForFunction(() => {
    const f = window.useApp?.getState?.().features
    return f?.length === 2 && f[1]?.type === 'shell'
  }, null, { timeout: 10000 })
  const feature = await page.evaluate(() => window.useApp.getState().features.at(-1))
  assert.equal(feature.thickness, 2)
  assert.equal(feature.kind, 'open')
  assert.ok(feature.nears?.length >= 1)
  console.log(JSON.stringify({ ok: true, target: baseURL, shell: feature }))
} finally {
  await browser.close()
}
