...
export const SKETCH_PANELS: Panel[] = [
  {
    name: 'CREATE',
    tools: [
      { id: 'sk_polyline', label: '直線', icon: 'sketch', glyph: '╱', shortcut: 'L', quick: true, tip: '直線/折線：連續點擊畫多段相連直線,回到起點或撳「閉合」成輪廓。畫紧嗰陣撳 A 鍵 = 喺 直線 / 相切弧 之間切換（可以直線接弧再接直線）。打數字 = 精確長度；方向自動吸 水平/豎直/45°/平行/垂直。靠近圓會吸住圓嘅【象限點（上下左右最遠點）】,由上一點拉向圓會吸住【切點】令該段同圓相切。' },
      ...
      { id: 'sk_cline', label: '構造線', icon: 'default', glyph: '┊', quick: true, tip: '構造參考線：點位置即落一條 豎直/水平 長虛線（工具面板切方向）。做對中參考、或做鏡像嘅【中心軸】。唔參與拉伸。' },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'sk_dim', label: '尺寸', icon: 'measure', glyph: '⟷', shortcut: 'D', quick: true, tip: '尺寸（D）：點一條邊 → 再點【放置位置】定方向（垂直偏置=對齊真長 · 左右放=豎直投影 · 上下放=水平投影）· 圓=直徑Ø · 兩個點=距離 · 點兩條邊（平行=間距 / 相交=夾角）。藍色標签可撳改數值（約束求解）；右鍵尺寸標签 = R↔Ø 切換 / 轉從動 / 刪除。過約束會弹框問【轉從動 / 取消】。' },
      { id: 'sk_c_h', label: '水平', icon: 'default', glyph: '━', sep: true, quick: true, tip: '水平：選 1 條邊（或 2 個點）→ 變水平。' },
      { id: 'sk_c_v', label: '豎直', icon: 'default', glyph: '┃', quick: true, tip: '豎直：選 1 條邊（或 2 個點）→ 變豎直。' },
      { id: 'sk_c_hv', label: '水平/豎直', icon: 'default', glyph: '┼', tip: '水平/豎直（合一，Fusion 單命令）：選一條或多條邊 → 按每條邊嘅方向自動判定水平定豎直施加（橫向→水平 / 縱向→豎直）。' },
      ...
      { id: 'sk_d_angle', label: '角度', icon: 'default', glyph: '∠', sep: true, tip: '角度尺寸：用選擇工具點 2 條邊 → 標夾角（度，可點改驱動幾何）。' },
