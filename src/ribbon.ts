...
const SURFACE: Panel[] = [
  { name: 'CREATE', tools: [
    ...
    { id: 'mergefaces', label: '合併面', icon: 'shell', tip: '合并面 Merge / Unify Same-Domain：点一个泊车曲面/壳 → 把同域（共面 / 共柱）相邻碎面合并成一张面（裁剪 / 缝合 / 导入后清噪、易拾、STEP 更小）。体积不变，纯拓扑合并。曲面分割的逆向清理。' },
...
const MESH: Panel[] = [
  ...
  { name: 'MODIFY', tools: [
    ...
    { id: 'compboolean', label: '组件布尔', icon: 'combine', quick: true, tip: '組件布尔（網格级）：先选目標組件（或唯一有几何組件），再点工具件 → 合併/切除/相交。亦可在 SOLID → ASSEMBLE 找到同名工具。' },
...
export const SKETCH_PANELS: Panel[] = [
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'sk_mirrory', label: '镜像', icon: 'mirror', glyph: '⇋', quick: true, tip: '镜像（Fusion 式 2 步）：①点中要镜像嘅輪廓（點一個=拣晒成条相连边，可多個）→「✓拣轴线」→ ②点一条现有【直线边/构造线】做镜像轴 → 反射（保留原件）。工具面板仲有「左右轴/上下轴」一键对称。' },
      { id: 'sk_mirrorx', label: '上下镜像', icon: 'mirror', glyph: '⇅', tip: '上下镜像（一键 across X 轴对称當前輪廓）。' },
      { id: 'sk_array', label: '阵列', icon: 'pattern', glyph: '▦', quick: true, tip: '阵列（Fusion 式）：㩒此入阵列模式 → 底栏面板設 矩形(行列+间距) / 环形(數量+角度+中心)，绿虚线实时预览 → 撳「应用阵列」。对【一個】輪廓。' },
      ...
      { id: 'sk_project', label: '投影几何', icon: 'default', glyph: '⧉', quick: true, tip: '投影几何：把實體嘅边「印」落當前草图做【真草图曲线】(蓝紫参考线) —— 印落嚟嘅线可标注/拉伸/修改,唔再只系参考。想沿住實體輪廓画嘢时用：撳掣后点實體嘅边即可。（要喺有實體嘅面、或穿过實體嘅平面开草图先印到）' },
      { id: 'sk_constr', label: '构造', icon: 'default', glyph: '╳', shortcut: 'X', tip: '构造几何（X）：把选中輪廓（或當前輪廓）转为虚线参考几何 — 可约束可吸附，唔参與拉伸。再按一次转返实线。' },
      { id: 'sk_guide', label: '扫掠导轨', icon: 'sweep', glyph: '⤳', tip: '导轨扫掠（Fusion guide rail）：先画一条折线做【导轨】→ 撳此记低（橙虚线）→ 再画扫掠【路径】→「沿路径扫掠」— 截面沿路径行进时跟住导轨转向。' },
      { id: 'sk_close', label: '闭合', icon: 'default', glyph: '✓', sep: true, quick: true, tip: '把折线/样条回到起點闭合成輪廓。' },
      { id: 'sk_dxf', label: '导出DXF', icon: 'default', glyph: '⤓', tip: '把當前草图輪廓導出为 2D DXF（激光切割 / AutoCAD）。' },
    ],
  },
  {
