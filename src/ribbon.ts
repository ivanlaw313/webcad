...
const SOLID: Panel[] = [
  {
    name: 'CREATE',
    tools: [
      { id: 'sketch', label: '建立草圖', icon: 'sketch', shortcut: 'Shift+S', quick: true, tip: '自由草圖：快速畫矩形/圓/折線等，滑鼠點擊即畫（新手首選）。㩒咗之後揀基準面 —— 紅XY/綠XZ/藍YZ 原點面，或直接㩒實體嘅任何一個平坦面。（S=命令搜索，同 Fusion 一樣）' },
      ...
      { id: 'revolve', label: '旋轉', icon: 'revolve', quick: true, tip: '旋轉：草圖輪廓绕一條軸旋轉成回轉體（軸/盤/車削件）。可加料/切割車槽/相交，薄壁>0 做燈罩/碗殼。' },
      ...
      { id: 'pipe', label: '管道', icon: 'pipeicon', tip: '管道：沿路徑掃出空心管（外徑+壁厚）。先畫路徑折線。' },
      {
        ...
        children: [
          ...
          { id: 'circpattern', label: '環形陣列', icon: 'pattern', tip: '绕任意軸等角度複製（Fusion 同款）：對象 = 整個實體或時間軸所選特征（螺栓孔圈/辐條），角度 = 完整360°/指定/對稱。' },
          { id: 'geopattern', label: '幾何陣列', icon: 'pattern', tip: '複製所選時間軸特征，並為每個實例指定平移或旋轉變換。' },
          { id: 'pathpattern', label: '路徑陣列', icon: 'pattern', tip: '先建實體→畫折線/樣條路徑→沿路徑等距複製實體。' },
        ],
      },
      { id: 'mirror', label: '鏡像', icon: 'mirror', quick: true, tip: '鏡像特征/實體：跨基准面（XY/XZ/YZ 或所選面）對稱複製選中嘅特征或整個實體。同「陣列」配對嘅常用建模操作。' },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'offsetplane', label: '偏移平面', icon: 'plane', quick: true, sep: true, tip: 'Fusion Offset Plane：先選原點 XY/XZ/YZ 基准面（或中間面），輸入距離建立平行參考面；如需由實體平面開始，可在方法選擇「偏移面（拾面+距離）」。' },
...
const LAB: Panel[] = [
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'autoorient', label: '自動擺正', icon: 'overhang', tip: '一鍵把零件旋轉到最省支撑嘅打印朝向（懸垂分析嘅建議方向），加一個可撤销嘅變換特征。' },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'scaleasm', label: '整體縮放', icon: 'move', tip: '整體縮放裝配：按比例放大/縮小成個多件設計（含相對間距，绕裝配中心）。' },
    ],
  },
  {
    name: '工具',
    tools: [
      { id: 'calc', label: '工程計算', icon: 'calc', quick: true, tip: '工程計算器：螺紋/配合/齒輪/皮帶/弹簧/軸承/折弯 等 14 個機械設計速算。' },
    ],
  },
  {
    name: '直接編輯擴展',
...
const SURFACE: Panel[] = [
  { name: 'CREATE', tools: [
    ...
    { id: 'surfrevolve', label: '曲面旋轉', icon: 'revolve', quick: true, tip: '曲面旋轉：開放截面绕軸旋成零厚旋轉曲面（燈罩/噴嘴/花瓶皮/涡輪毂），無需先做實體再殼。先畫開放折線截面 → 撳此 → 輸角度,軸(Y/X)。出獨立曲面體（要實體再「加厚」/「縫合」）。' },
    ...
    { id: 'rotateface', label: '旋轉面', icon: 'replaceface', tip: '旋轉面：點一個【平面】→ 輸入铰軸(X/Y/Z) + 角度 → 該面绕住經過點擊點嘅铰線倾斜,鄰面自動癒合成梯形（實體保留）。改導入件/斜面角度用。僅限平面。' },
...
export const SKETCH_PANELS: Panel[] = [
  {
    ...
    tools: [
      ...
      { id: 'sk_mline', label: '中點線', icon: 'sketch', glyph: '┿', tip: '中點線（Fusion Midpoint Line）：第一點 = 中點，第二點 = 一端 → 由中點向兩端對稱嘅直線段（做對稱基准/中心參照）。打數字 = 全長。' },
      ...
      { id: 'sk_circle3', label: '三點圓', icon: 'cylinder', glyph: '◓', tip: '三點圓：點三點定圓（外接圓）— 對齊已有特征/孔位方便。' },
      ...
      { id: 'sk_text', label: '文字', icon: 'default', glyph: 'T', tip: '草圖文字（Fusion 流程一步到位）：輸入 文字/字号/凸高 → 凸字（切割模式=刻字）特征，時間軸可改。' },
    ],
  },
  {
    name: 'MODIFY',
    tools: [
      ...
      { id: 'sk_scale', label: '縮放', icon: 'scale', glyph: '⤢', tip: '草圖縮放（Fusion Sketch Scale）：選中輪廓（無選擇=全部）绕形心乘系數 k（>1 放大 / <1 縮小；真圓弧/椭圓半徑同步縮放）。快捷鍵喺草圖內可用（M=移動 / 縮放喺 ribbon）。' },
