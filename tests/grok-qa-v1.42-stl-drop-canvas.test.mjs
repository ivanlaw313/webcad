...
test('shouldAllowMeshDragOver accepts Files + empty-types automation drags', () => {
  ...
  assert.equal(shouldAllowMeshDragOver({ types: ['text/plain'], effectAllowed: 'copy' }), false)
...
test('Viewport wires capture-phase mesh drop (BUG-BD-4101)', () => {
  ...
  assert.match(viewport, /addEventListener\('dragover',\s*onDragOver,\s*true\)/)
  assert.match(viewport, /addEventListener\('drop',\s*onDrop,\s*true\)/)
  assert.match(viewport, /shouldAllowMeshDragOver/)
  assert.match(viewport, /resolveMeshDropFile/)
...
test('App shell wires capture-phase mesh drop', () => {
  ...
  assert.match(app, /addEventListener\('dragover',\s*onDragOver,\s*true\)/)
  assert.match(app, /shouldAllowMeshDragOver/)
  assert.match(app, /resolveMeshDropFile/)
