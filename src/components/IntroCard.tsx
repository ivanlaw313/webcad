    ...
    <div className="drawing-overlay" onClick={dismiss}>
      <div className="help-modal intro-card-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 540 }}>
        ...
        <div className="intro-card-body" style={{ padding: '14px 18px', fontSize: 14, color: '#222', lineHeight: 1.75 }}>
          ...
          <div style={step}><div style={num}>3</div><div><b>出图 / 导出</b><br />撳「<b>工程图</b>」出三视图 + 尺寸/孔表标注；或导出 <b>STL / STEP / glTF</b> 攞去 3D 打印或分享。</div></div>
          <div style={{ ...step, color: '#555' }}><div style={{ ...num, background: '#5a7' }}>✎</div><div><b>想自己画？</b><br />流程只需四步：撳「<b>创建草图</b>」→ 喺 3D 拣一个面（高亮 <b style={{ color: '#d6694e' }}>红 XY</b>/<b style={{ color: '#4e9e5e' }}>绿 XZ</b>/<b style={{ color: '#4e7fd6' }}>蓝 YZ</b> 或实体平面）→ 画 矩形/圆/多边形（打数字 = 精确尺寸）→「完成草图」→「<b>拉伸</b>」出实体。想加尺寸 / 约束？主草图已内建：撳草图栏「<b>尺寸</b>」(D) 点条边打数值即驱动几何（Esc 取消 / Ctrl+Z 撤销）。<br /><span style={{ color: '#2e7d32' }}>👉 第一次用？撳下面「<b>🎓 手把手教学</b>」，我一步步带你整出第一个零件。</span></div></div>
