...
const SOLID: Panel[] = [
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'planemid', label: '中間平面', icon: 'plane', tip: 'Fusion Midplane：拾取两个平面或平坦面，在两者正中建立参考面。' },
      { id: 'planeperp', label: '垂直平面', icon: 'plane', tip: 'Fusion Perpendicular Plane：拾取平面及参考边，在指定距离建立垂直平面。' },
      { id: 'plane2edge', label: '過兩邊平面', icon: 'plane', tip: 'Fusion Plane Through Two Edges：顺序拾取两条边，建立同时包含两边的参考面。' },
      { id: 'plane3pt', label: '過三點平面', icon: 'plane', tip: 'Fusion Plane Through Three Points：用最后三个构造点建立参考面。' },
      { id: 'planepath', label: '沿路徑平面', icon: 'plane', tip: 'Fusion Plane Along Path：拾取路径并以 0–1 距离定位，建立垂直于路径切向的平面。' },
      { id: 'axiscyl', label: '圓柱／圓錐／環面軸', icon: 'axis', quick: true, sep: true, tip: 'Fusion Axis Through Cylinder/Cone/Torus：拾取旋转面建立其中心轴。' },
      { id: 'axisperpface', label: '垂直面軸', icon: 'axis', tip: 'Fusion Axis Perpendicular To Face：拾取平面上的一点，建立沿面法向的构造轴。' },
      { id: 'axis2planes', label: '過兩平面軸', icon: 'axis', tip: 'Fusion Axis Through Two Planes：拾取两个不平行平面，以交线建立构造轴。' },
      { id: 'axis2pt', label: '過兩點軸', icon: 'axis', tip: 'Fusion Axis Through Two Points：用最后两个构造点建立任意方向轴。' },
      { id: 'axisedge', label: '沿邊軸', icon: 'axis', tip: 'Fusion Axis Through Edge：拾取一条直边，以该边建立构造轴。' },
      { id: 'pointvertex', label: '頂點構造點', icon: 'cpoint', quick: true, sep: true, tip: 'Fusion Point At Vertex：拾取一条边，在最近端点建立构造点。' },
      { id: 'point2edges', label: '兩邊交點', icon: 'cpoint', tip: 'Fusion Point Through Two Edges：拾取两条边，在交点或最近逼近点建立构造点。' },
      { id: 'point3planes', label: '三平面交點', icon: 'cpoint', tip: 'Fusion Point Through Three Planes：拾取三个平面，在唯一公共交点建立构造点。' },
      { id: 'pointcenter', label: '圓／球／環面中心點', icon: 'cpoint', tip: 'Fusion Point At Center Of Circle/Sphere/Torus：拾取圆形边，在拟合中心建立构造点。' },
      { id: 'pointedgeplane', label: '邊與平面交點', icon: 'cpoint', tip: 'Fusion Point At Edge And Plane：拾取边及平面，在交点建立构造点。' },
      { id: 'pointpath', label: '沿路徑點', icon: 'cpoint', tip: 'Fusion Point Along Path：拾取路径，以 0–1 比例或毫米距离定位构造点。' },
    ],
  },
  {
    name: 'INSPECT',
  ...
  },
  {
    name: 'INSERT',
    tools: [
      { id: 'insertcomponent', label: '插入組件', icon: 'component', quick: true, tip: 'Fusion Insert Component：从 STEP/STP 选取零件或装配，作为独立组件插入当前设计；保留多零件的相对位置。当前不建立 Fusion 云端/F3D 的关联链接。' },
      { id: 'insertfastener', label: '插入緊固件', icon: 'component', quick: true, tip: 'Fusion Insert Fastener 工作流：选内建 ISO 紧固件类型、M 规格与长度后插入独立装配组件。几何尺寸真实；螺纹以简化光杆表示，不伪称 Autodesk 云端供应商库。' },
      { id: 'insertmesh', label: '插入STL网格', icon: 'importmesh', quick: true },
      { id: 'insert3mf', label: '插入3MF網格', icon: 'importmesh', quick: true, tip: 'MakerWorld / Printables 下载嘅 3MF 直接导入做组件——多零件保留摆位同颜色，单位自动转 mm。' },
      { id: 'insertobj', label: '插入OBJ網格', icon: 'importmesh', quick: true, tip: 'Wavefront OBJ 网格导入（Blender、扫描或网上模型常用）作参考组件。' },
      { id: 'insertcanvas', label: '畫布', icon: 'importmesh', sep: true, tip: 'Fusion Canvas：选择参考图片，放到当前草图平面作为描摹底图；加入后可调宽度、透明度、位置、旋转、翻转及两点标定比例。' },
      { id: 'insertdecal', label: '貼花', icon: 'importmesh', tip: 'Fusion Decal：选择 logo／标签图片，然后点击实体表面放置；可调尺寸、旋转、透明度及翻转，并随项目保存。' },
...
const LAB: Panel[] = [
  {
    ...
    tools: [
      ...
      { id: 'othread', label: '面加螺紋', icon: 'thread', tip: 'WebCAD 扩充：在圆柱面建立真实外螺纹。' },
      { id: 'ithread', label: '內螺紋孔', icon: 'hole', tip: 'WebCAD 扩充：建立真实内螺纹孔。' },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'tube', label: '圓管', icon: 'cylinder', quick: true, tip: '圆管/衬套：设外径Ø/壁厚/高建空心管。' },
      { id: 'rbox', label: '圓角盒', icon: 'box', quick: true, tip: '圆角长方体/外壳盒：长方体四条竖边倒圆角。电子外壳常用。' },
      { id: 'wedge', label: '楔形', icon: 'box', tip: '楔形/斜坡：设长/宽/高建一端高、另一端削平的三角块。' },
      { id: 'dome', label: '圓頂', icon: 'sphere', tip: '圆顶/半球：设直径建半球；冠高<半径 出浅球冠（镜片/表镜/按钮）。' },
      { id: 'halfcyl', label: '半圓柱', icon: 'cylinder', tip: '半圆柱/D 形：圆柱切一半（D 形截面）。D 形轴/D 孔常用。' },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'splitsketch', label: '草圖輪廓分割', icon: 'split', tip: 'WebCAD 扩展：用封闭草图轮廓贯穿分割实体。' },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'planeparpt', label: '過點平行面', icon: 'plane', tip: '以最后一个构造点及所拾平面建立平行参考面。' },
      { id: 'caxis', label: '方向構造軸', icon: 'axis', tip: '以 X／Y／Z 方向及指定坐标建立构造轴。' },
      { id: 'cpoint', label: '坐標構造點', icon: 'cpoint', tip: '输入 X／Y／Z 坐标建立构造点。' },
      { id: 'midcpoint', label: '兩點中點', icon: 'cpoint', tip: '在最后两个构造点正中建立新构造点。' },
      { id: 'cptgrid', label: '構造點陣列', icon: 'cpoint', tip: '一次建立矩形或极坐标构造点阵列。' },
      { id: 'projsurf', label: '投影到圓柱面', icon: 'axis', tip: '把平面草图路径按弧长映射到可展圆柱面，产生 3D 参考曲线。' },
    ],
  },
]
...
const MESH: Panel[] = [
  ...
  { name: 'INSERT', tools: [
    ...
    { id: 'insert3mf', label: '插入3MF網格', icon: 'importmesh', quick: true, tip: 'MakerWorld / Printables 下载嘅 3MF 直接导入做组件——多零件保留摆位同颜色，单位自动转 mm。' },
    { id: 'insertobj', label: '插入OBJ網格', icon: 'importmesh', quick: true, tip: 'Wavefront OBJ 网格导入（Blender / 扫描 / 网上模型常用）作参考组件。' },
  ] },
  g('ASSEMBLE'),
  { name: 'SELECT', tools: [{ id: 'select', label: '選擇', icon: 'select', quick: true, tip: '選擇工具：撳零件/面/边就拣中佢（撳空白处 = 唔拣）；撳 Del 键删除所选。' }] },
  { name: 'EXPORT', tools: [
