...
const stepGeometry=buffer=>new TextDecoder().decode(buffer).split('DATA;')[1].replace(/Open CASCADE STEP translator 7\.6 \d+/g,'Open CASCADE STEP translator 7.6').replace(/NEXT_ASSEMBLY_USAGE_OCCURRENCE\('\d+'/g,"NEXT_ASSEMBLY_USAGE_OCCURRENCE('export-id'")
...
async function seed({dependent=false,seedBinding=true}={}){
 ...
 useApp.setState({sketchProfiles:[{type:'circle',c:[0,0],r:3},{type:'circle',c:[20,0],r:3}],sketchShape:null,extrudeHeight:5,sketchOp:'new'})
 await g().extrudeSketch();assert.equal(g().mode,'model',g().status)
 const feature=g().features.find(f=>f.type==='extrude'||f.type==='extgroup');assert.ok(feature);assert.equal(feature.type,'extgroup','fixture must create a true multiple-profile extrusion')
...
async function nativeAt(z){
 ...
 try{
  assert.ok(analyzer.IsValid_2());assert.ok(Math.abs(measureVolume(shape)-Math.PI*9*5*2)<1e-6)
...
test('multiple-profile datum sketch entry records the actual construction-plane dependency',async()=>{
 const {id}=await seed({seedBinding:false})
 assert.deepEqual(g().sketchSources[id].datumRef,{idx:0,base:'XY'})
})
for(const dependent of [false,true])test(`multiple-profile datum offset parameter propagates while ${dependent?'dependent':'unrelated'} draft remains open`,async()=>{
