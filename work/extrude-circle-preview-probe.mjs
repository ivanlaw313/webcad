...
const cut={id:'cut',type:'extrude',profile:{kind:'circle',c:[10,10],r:2},height:20,baseZ:0,operation:'cut',toFace:{near:[5,5,20]}};
...
test('to-face cut → distance preview → commit → Undo/Redo → JSON retains matching depth',async()=>{
 useApp.setState({...useApp.getInitialState()},true);assert.equal(await useApp.getState().applyFeatures([base,cut],'base'),true);
 const before=await volume(),count=useApp.getState().undoStack.length;assert.ok(Math.abs(before-(8000-Math.PI*4*20))<.001);
 ...
 const preview=await useApp.getState().previewExtrudeEdit();assert.ok(preview&&!preview.failed?.length);const expected=8000-Math.PI*4*5;
 console.log({previewVolume:meshVolume(preview),expected,params:useApp.getState().featDlg.params});assert.ok(Math.abs(meshVolume(preview)-expected)<1,'tessellated preview matches blind cut');assert.ok(Math.abs(await volume()-before)<.001);assert.equal(useApp.getState().undoStack.length,count);
