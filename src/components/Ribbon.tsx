          ...
          {fileMenu && createPortal(
            <>
              ...
              <div ref={fileMenuRef} role="menu" aria-label="文件 File" onKeyDown={(e) => {
                ...
                <div className="panel-menu-divider" />
                <button type="button" role="menuitem" className="panel-menu-item" onClick={() => void exportStl()}><ToolIcon name="save" size={16} />{en ? 'Export STL' : '导出 STL'}</button>
                ...
                <button type="button" role="menuitem" className="panel-menu-item" onClick={() => void generateDrawing()}><ToolIcon name="drawing" size={16} />{en ? 'Drawing (3 Views)' : '工程图（三视图）'}</button>
                <button type="button" role="menuitem" className="panel-menu-item" title="装配三视图 + 气泡编号 + BOM 表（组件网格投影：轮廓+特征边）" onClick={() => void useApp.getState().generateAsmDrawing()}><ToolIcon name="drawing" size={16} />{en ? 'Assembly Drawing + BOM' : '装配工程图 + BOM'}</button>
