...
const FD_TITLE: Record<string, string> = { gearbox: '齒輪箱向導', worm: '蝸桿', crowngear: '冠齒輪', automatedmodel: 'Automated Modeling · Connector v1', pattern: '矩形陣列', cpattern: '環形陣列', circpattern: '環形陣列', pathpattern: '路徑陣列', mirror: '鏡像', move: '移動/複製', scale: '縮放', draft: '拔模', revolve: '旋轉', rib: '加強筋/腹板', pipe: '管道', box: '長方體', cylinder: '圓柱', sphere: '球', torus: '圓環', cone: '圓錐/圓台', wedge: '楔形', dome: '圓頂', halfcyl: '半圓柱', pie: '扇形柱', tube: '圓管/襯套', rtube: '方管', profile: '型材', rbox: '圓角盒', prism: '多邊形棱柱', pyramid: '棱錐', coil: '螺旋', thread: '螺紋桿', cylpatch: '曲面貼花', sheetmetal: '鈑金件', gear: '齒輪', rack: '齒條', pulley: 'V帶輪', plane: '參考平面', cpoint: '構造點', caxis: '構造軸', combine: '合併/布爾', splitbody: '分割實體', 'extrude-edit': '拉伸', 'fillet-edit': '圓角', 'chamfer-edit': '倒角', 'shell-edit': '抽殼' }
...
function PatternPreview() {
  ...
  if (k === 'pattern') {
    ...
    // T757 修：部分弧 worker 用 angle/(n−1)（端點含），预览以前永远 /n — 同步返先唔会预览同实际唔一致
...
// S112：换 Google「turbo」感知均勻配色（多项式拟合，无依赖）取代 jet —— jet 喺绿黄段有亮度伪边/误读，
...
function MoldOverlay() {
  ...
  useEffect(() => {   // 颜色：场/滑杆变都只改色
    ...
    // 归一化范围 [smn,smx]：smx=shrinkMax；smn 由均勻度反推（moldsolve：uniformity=shrinkMin/shrinkMax）→ O(1) 且同图例标签一致。未填(NaN)灰。
...
function LenInput({ mm, onMm, unit, w = 56, title }: { mm: number; onMm: (v: number) => void; unit: LenUnit; w?: number; min?: number; title?: string }) {
  ...
  return (
    <input
      ...
      // BUG-SO18F-001: notify parent even when below min (Ø-1) so preview/confirm can reject+clear instead of keeping a stale legal value.
      onChange={(e) => { setTxt(e.target.value); const v = parseLen(e.target.value, unit); if (v != null) onMm(v) }}
      style={{ width: w }}
    />
  )
}
...
export default function Viewport() {
  ...
  const snapTypes = useApp((s) => s.snapTypes)   // GM-W8 β2-#A2：逐类捕捉开关（端點/中点/圆心/象限/交点/切点）
  ...
  return (
    ...
    >
      ...
      >
        ...
        <MeasureSnapMarkers />{/* #174-6：测量捕捉点标记（特征边端點/中点） */}
      ...
      {ctxMenu && (() => {
        ...
        const repeat: MMItem = {
          key: 'repeat', glyph: '↻', label: lastCommand ? `重複·${lastCommand.label}` : '重複上次',
        ...
        if (mode === 'sketch' && skSelN > 0) {
          ...
          sectors = [
            repeat,
            { key: 'del', glyph: '🗑', label: msg('mm.delete', lang), fn: () => useApp.getState().skDeleteSel() },
            ...
            { key: 'move', glyph: '✥', label: msg('mm.moveCopy', lang), fn: () => useApp.getState().startSkMove() },
            { key: 'extrude', glyph: '⬆', label: msg('mm.extrudeEllipsis', lang), disabled: !(sketchShape || sketchProfiles.length > 0), fn: () => openExtrudeDlg() },
            { key: 'undo', glyph: '↶', label: msg('mm.undo', lang), fn: () => void useApp.getState().undo() },
            { key: 'esc', glyph: '✕', label: msg('mm.clearSel', lang), fn: () => { useApp.getState().escSketch() } },
          ]
          overflow = [
            { key: 'mirror', label: '◑ ' + msg('mm.mirror', lang), fn: () => useApp.getState().runCommand('sk_mirrory', '鏡像') },
            { key: 'offset', label: '⇢ ' + msg('mm.offset', lang), fn: () => useApp.getState().runCommand('sk_offset', '偏移') },
            ...
            { key: 'fix', label: '⚓ ' + msg('mm.fix', lang), fn: () => useApp.getState().addSkCon('fix') },
            { key: 'skarr', label: '▦ ' + msg('mm.skArr', lang), fn: () => useApp.getState().runCommand('sketcharray', '陣列輪廓') },
            'sep',
          ...
          sectors = [
            repeat,
            { key: 'line', glyph: '╱', label: msg('mm.line', lang), fn: () => setSketchTool('polyline') },
            { key: 'rect', glyph: '▭', label: msg('mm.rect', lang), fn: () => setSketchTool('rectangle') },
            { key: 'circle', glyph: '◯', label: msg('mm.circle', lang), fn: () => setSketchTool('circle') },
            { key: 'finish', glyph: '✓', label: msg('mm.finishSketch', lang), fn: () => finishSketch() },
            { key: 'extrude', glyph: '⬆', label: msg('mm.extrudeEllipsis', lang), disabled: !(sketchShape || sketchProfiles.length > 0), fn: () => openExtrudeDlg() },
            { key: 'undo', glyph: '↶', label: msg('mm.undo', lang), fn: () => void useApp.getState().undo() },
          ...
          overflow = [
            ...
            { key: 'crect', label: '⊞ ' + msg('mm.crect', lang), fn: () => setSketchTool('crect') },
            { key: 'circle2p', label: '⊘ ' + msg('mm.circle2p', lang), fn: () => setSketchTool('circle2p') },
            { key: 'circle3', label: '◓ ' + msg('mm.circle3', lang), fn: () => setSketchTool('circle3') },
            { key: 'polygon', label: '⬡ ' + msg('mm.polygon', lang), fn: () => setSketchTool('polygon') },
            ...(sketchShape || sketchProfiles.length > 0 ? [{ key: 'skarr', label: '▦ ' + msg('mm.skArr', lang), fn: () => useApp.getState().runCommand('sketcharray', '陣列輪廓') } as MMItem] : []),
            'sep',
            { key: 'tgfill', label: `${skView.fill ? '☑' : '☐'} 輪廓填充`, fn: () => setSkView({ fill: !skView.fill }) },
            { key: 'tgannot', label: `${skView.annot ? '☑' : '☐'} 尺寸標註`, fn: () => setSkView({ annot: !skView.annot }) },
            { key: 'tgcons', label: `${skView.cons ? '☑' : '☐'} 約束徽章`, fn: () => setSkView({ cons: !skView.cons }) },
          ...
          const del: MMItem = { key: 'del', icon: 'trash', label: msg('mm.delete', lang), disabled: !selectedFeature && !selectedComponent, fn: () => { if (selectedFeature) { removeFeature(selectedFeature); selectFeature(null) } else if (selectedComponent) useApp.getState().deleteComponent(selectedComponent) } }
          const fit: MMItem = { key: 'fit', glyph: '⊕', label: msg('mm.fit', lang), fn: () => requestFit() }
          const newSketch: MMItem = { key: 'sketch', icon: 'sketch', label: msg('mm.createSketch', lang), fn: () => startSketch() }
          if (bodyMesh) {
            // ── 实体上（有活动实体）：N 重复 · NE 草图 · E 按拉 · SE 测量 · S 删除 · SW 移动 · W 圆角 · NW 适应
            sectors = [
              repeat,
              newSketch,
              { key: 'presspull', icon: 'presspull', label: msg('mm.pressPull', lang), fn: () => runCommand('presspull', '按拉') },
              { key: 'measure', icon: 'measure', label: msg('mm.measure', lang), fn: () => runCommand('measure', '測量') },
              del,
              { key: 'move', icon: 'move', label: msg('mm.moveCopy', lang), fn: () => runCommand('move', '移動') },
              { key: 'fillet', icon: 'fillet', label: msg('mm.fillet', lang), fn: () => runCommand('fillet', '圓角') },
              fit,
            ]
          } else {
          ...
          overflow = [
            ...(bodyMesh ? [
              { key: 'delface', label: '⌦ ' + msg('mm.delFace', lang), fn: () => runCommand('delface', '刪面') } as MMItem,
            ] : [
              { key: 'measure', label: '📏 ' + msg('mm.measure', lang), fn: () => runCommand('measure', '測量') } as MMItem,
            ]),
            ...(bodyMesh ? [
              { key: 'chamfer', label: '◣ ' + msg('mm.chamfer', lang), fn: () => runCommand('chamfer', '倒角') } as MMItem,
              { key: 'facefillet', label: '⌒ ' + msg('mm.faceFillet', lang), fn: () => runCommand('facefillet', '面圓角') } as MMItem,
              { key: 'shell', label: '⬚ ' + msg('mm.shell', lang), fn: () => runCommand('shell', '抽殼') } as MMItem,
            ...
            ...(sc ? ['sep' as const,
              ...
              { key: 'cremesh', label: `▦ 重網格「${sc.name}」`, fn: async () => { const v = await useApp.getState().appPrompt('各向同性重网格（Botsch-Kobbelt：均勻边长 + 原面重投影保形）\n目标边长 mm（细=三角多更平滑，粗=三角少）', '3'); if (v == null) return; void useApp.getState().remeshComponentMesh(sc.id, Number(v) || 3) } } as MMItem,
            ...
            ...(components.filter((c) => !c.hidden).length > 1 ? ['sep' as const,
              { key: 'stack', label: '⊟ ' + msg('mm.stackVert', lang), fn: () => useApp.getState().stackComponents() } as MMItem,
      ...
      {edgeRoundPick && (
        <CommandDialog
          ...
          width={300}
          okLabel={edgeRoundPick === 'chamfer' ? 'OK' : `確定（${edgeRoundPicks.length}）`}
          ...
          summary={edgeRoundPick === 'fillet' ? <>{filletType === 'rule' ? `規則圓角 R${edgeRoundSize}` : filletType === 'full' ? '全圓角' : filletMode === 'asymmetric' ? `不對稱圓角 ${edgeRoundSize}/${filletR2}` : `圓角 R${edgeRoundSize}`} · {edgeRoundPicks.length || 0} {filletType === 'rule' ? '面／特徵' : filletType === 'full' ? '面' : '條稜'}</> : null}
        >
          ...
          </> : <SelectionChip label="邊／面／特徵" count={edgeRoundPicks.length} hint="逐條點選要處理的稜（成條高亮；再點同一條=取消）" onClear={() => useApp.getState().clearEdgeRoundPicks()} />}
          {edgeRoundPick === 'fillet' && filletType === 'fillet' && filletMode !== 'chord' && (
            <div style={{ border: '1px solid #c8d0d8', borderRadius: 3, padding: 4 }} title="Fusion Radius Group：每組有自己嘅選邊槽、半徑同連續性；點擊一行後，新揀嘅邊會加入該組。">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3, maxHeight: 132, overflowY: 'auto' }}>
                {filletRadiusGroups.map((g, gi) => {
                  ...
                  return <div key={`frg-${gi}`} onClick={() => useApp.getState().selectFilletRadiusGroup(gi)} style={{ display: 'grid', gridTemplateColumns: '54px 62px 1fr', gap: 4, alignItems: 'center', padding: '3px 4px', borderRadius: 3, cursor: 'pointer', background: active ? '#dceeff' : 'transparent', outline: active ? '1px solid #4b91d1' : '1px solid transparent' }}>
                    ...
                    <span><input type="number" aria-label={filletMode === 'asymmetric' ? `半徑組 ${gi + 1} 偏移 1 mm` : `半徑組 ${gi + 1} 半徑 mm`} min={0} step={0.5} value={g.radius} onClick={(e) => e.stopPropagation()} onFocus={() => useApp.getState().selectFilletRadiusGroup(gi)} onChange={(e) => useApp.getState().setFilletGroupRadius(gi, Number(e.target.value))} style={{ width: 48 }} /> mm</span>
                    <select aria-label={`半徑組 ${gi + 1} 連續性`} value={filletMode === 'asymmetric' ? 'G1' : g.continuity} disabled={filletMode === 'asymmetric'} title={filletMode === 'asymmetric' ? 'Fusion 360 不對稱圓角固定使用 G1 連續性' : undefined} onClick={(e) => e.stopPropagation()} onFocus={() => useApp.getState().selectFilletRadiusGroup(gi)} onChange={(e) => useApp.getState().setFilletGroupContinuity(gi, e.target.value as 'G1' | 'G2')} style={{ height: 24, minWidth: 92 }}><option value="G1">相切 G1</option><option value="G2">曲率 G2</option></select>
                  </div>
                })}
              </div>
              <div style={{ display: 'flex', gap: 4, marginTop: 4 }}><button type="button" aria-label="新增半徑組" onClick={() => useApp.getState().addFilletRadiusGroup()} style={{ minWidth: 28 }}>＋</button><button type="button" aria-label="刪除半徑組" disabled={filletRadiusGroups.length <= 1} onClick={() => useApp.getState().removeFilletRadiusGroup()} style={{ minWidth: 28 }}>×</button><span style={{ fontSize: 10, color: '#6b7680', alignSelf: 'center' }}>選中第 {filletActiveGroup + 1} 組</span></div>
            </div>
          )}
          ...
          {edgeRoundPick === 'fillet' && filletType === 'fillet' && filletMode === 'chord' ? (
            ...
            <label>
              <span style={{ color: '#6b7680' }}>半徑</span>
              <span><input type="number" aria-label="圓角半徑 mm" min={0.5} step={0.5} value={edgeRoundSize} onChange={(e) => setEdgeRoundSize(Number(e.target.value))} style={{ width: 66 }} /> mm</span>
            </label>
          ) : null}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 120, overflowY: 'auto' }}>
                {edgeRoundPicks.map((_, i) => (
                  <div key={'err' + i} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                    <span style={{ width: 38, color: '#8a97a2' }}>稜 {i + 1}</span>
                    <input type="number" aria-label={`稜 ${i + 1} 距離 mm`} min={0.1} step={0.5} value={+(edgeRoundRadii[i] ?? edgeRoundSize).toFixed(2)} onChange={(e) => useApp.getState().setEdgeRoundRadiusAt(i, Number(e.target.value))} style={{ width: 60 }} /> mm
                  </div>
                ))}
              </div>
            </div>
      ...
      {surfBridgePickM && (
        <CommandDialog
          ...
          width={320}
          okLabel={`確定（${surfBridgePicks.length}/2）`}
      ...
      {boundaryPatchPick && (
        <CommandDialog
          ...
          title="边界补面"
          width={320}
          okLabel={`確定（${boundaryPatchPicks.length}）`}
      ...
      {faceFilletMode && (
        <CommandDialog
          ...
          title="面圓角"
          width={244}
          okLabel={`確定（${faceFilletPicks.length}/2）`}
          okDisabled={faceFilletPicks.length !== 2}
          okTip="面圓角（Enter）"
          onOk={() => void commitFaceFillet()}
          onCancel={() => cancelFaceFillet()}
          summary={<>面圓角 R{faceFilletRadius} · {faceFilletPicks.length}/2 面</>}
        >
          ...
          <label>
            <span style={{ color: '#6b7680' }}>半徑</span>
            <span><input type="number" aria-label="面圓角半徑 mm" min={0.1} step={0.5} value={faceFilletRadius} onChange={(e) => setFaceFilletRadius(Number(e.target.value))} style={{ width: 66 }} /> mm</span>
          </label>
              ...
              {moldPacking && (
                <>
                  <div style={{ display: 'flex', gap: 10, margin: '1px 0 4px', paddingLeft: 18, fontSize: 11, color: '#6b7680' }}>
                    ...
                    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }} title="保压时间（s；典型 2–10）。越长越多 CV 喺高压下冻结锁压 → 收缩越均勻。">
                      时间 s
                  ...
                  <div style={{ fontSize: 10.5, color: '#6b7680', paddingLeft: 18, margin: '-2px 0 4px', lineHeight: 1.4 }}>
                    唔使开热耦合都用得：热耦合关 → 保压期由【均勻熔温】起算靠壁面导热冻结；开 → 用真末温度场（更贴近）。
                  </div>
                </>
              )}
            </>
            ...
            <>
              ...
              <div style={{ fontSize: 11, lineHeight: 1.5 }}>
                {moldTempView && moldResult.thermalUsed ? (
                  ...
                  // GM-P3UI：缩水率色标 —— 同熔体温度款 蓝(低)→红(高)；min/max 标 %（2 位小数），min 由均勻度反推（同 overlay 一致）
                  <>
                    <div style={{ height: 10, borderRadius: 3, background: 'linear-gradient(90deg,rgb(0,26,255),rgb(128,26,128),rgb(255,26,0))' }} />
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: '#6b7680' }}>
                ...
                {moldResult.packing && (
                  // GM-P3UI：Stage-3 保压 / PVT 收缩摘要 —— 平均/最大/均勻度 徽章 + packTraceNote 诚实小注（切「缩水率」云图睇分布）
                  <div style={{ color: '#3aa8c0' }}>
                    ⏱ 保压 Stage-3：平均缩水 {moldResult.packing.shrinkAvg.toFixed(2)}% · 最大 {moldResult.packing.shrinkMax.toFixed(2)}% · 均勻度 {(moldResult.packing.shrinkUniformity * 100).toFixed(0)}%
      ...
      {featDlg && (
        <CommandDialog icon={featDlg.kind === 'geoPattern' ? 'pattern' : (FD_ICON[featDlg.kind] ?? 'default')} title={tStatus((featDlg.editId ? '編輯 · ' : '') + (featDlg.kind === 'geoPattern' ? '幾何陣列' : (FD_TITLE[featDlg.kind] ?? featDlg.kind)), lang)} okDisabled={
          ...
          {featDlg.editId && <div style={{ fontSize: 11, color: '#8a97a2', marginBottom: 4 }}>{tStatus('編輯模式：改參數 → 確定重建；稜/面選擇集及輪廓保留原值', lang)}</div>}
          ...
          {featDlg.kind === 'pattern' && (<>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
              <span>{tStatus('對象', lang)}</span>
              {String(featDlg.params.objectType ?? 'bodies') === 'features'
                ? <span style={{ color: cpSelFeat ? '#2c7' : '#c60' }}>{cpSelFeat ? `✓ ${tStatus('已選特徵', lang)}` : tStatus('在時間軸選擇特徵', lang)}</span>
                ...
                : <button className="cs-btn" onClick={() => setFeatParam('objectPicked', 1)}>{+featDlg.params.objectPicked ? `✓ ${tStatus('活動實體', lang)}` : tStatus('選擇', lang)}</button>}
            </div>
            <label title={tStatus('面陣列會建立獨立 B-rep 曲面副本；之後可用 Stitch 或 Thicken 轉成後續幾何。', lang)}>{tStatus('對象類型', lang)} <select value={String(featDlg.params.objectType ?? 'bodies')} onChange={(e) => { const v = e.target.value; setFeatParam('objectType', v); setFeatParam('target', v === 'features' ? 'feature' : 'body') }} style={{ height: 26 }}><option value="bodies">{tStatus('整個實體', lang)}</option><option value="faces">{tStatus('面', lang)}</option><option value="features">{tStatus('所選特徵', lang)}</option><option value="components">{tStatus('組件', lang)}</option></select></label>
            {String(featDlg.params.objectType ?? 'bodies') === 'components' && <span style={{ fontSize: 11, color: cpSelCompCount ? '#2c7' : '#c60' }}>{cpSelCompCount ? `✓ ${tStatus('已選組件', lang)} ×${cpSelCompCount}（${tStatus('共享定義的 occurrence 陣列', lang)}）` : tStatus('← 先喺裝配樹勾選 ☑ 或畫布選中組件', lang)}</span>}
            {featDlg.params.target === 'feature' && <span style={{ fontSize: 11, color: cpSelFeat ? '#2c7' : '#c60' }}>{cpSelFeat ? `✓ ${tStatus('已選特徵', lang)} ×${useApp.getState().selectedFeatures.length || 1}` : msg('ui.clickFeatureFirst', lang)}</span>}
            <label title={tStatus('距離類型（Fusion Distance Type）：間距 = 相鄰副本距離；總長 = 首末副本總跨距（間距自動 = 總長÷(數量−1)）', lang)}>{tStatus('距離', lang)} <select value={String(featDlg.params.dtype ?? 'spacing')} onChange={(e) => setFeatParam('dtype', e.target.value)} style={{ height: 26 }}><option value="spacing">{tStatus('間距', lang)}</option><option value="extent">{tStatus('總長', lang)}</option></select></label>
            <label>{tStatus('X數量', lang)} <input type="number" min={1} step={1} value={featDlg.params.countX} onChange={(e) => setFeatParam('countX', Number(e.target.value))} style={{ width: 46 }} /></label>
            <label>{tStatus(featDlg.params.dtype === 'extent' ? 'X總長' : 'X間距', lang)} <input type="number" step={5} value={featDlg.params.dx} onChange={(e) => setFeatParam('dx', Number(e.target.value))} style={{ width: 56 }} /></label>
            <label>{tStatus('Y數量', lang)} <input type="number" min={1} step={1} value={featDlg.params.countY} onChange={(e) => setFeatParam('countY', Number(e.target.value))} style={{ width: 46 }} /></label>
            <label>{tStatus(featDlg.params.dtype === 'extent' ? 'Y總長' : 'Y間距', lang)} <input type="number" step={5} value={featDlg.params.dy} onChange={(e) => setFeatParam('dy', Number(e.target.value))} style={{ width: 56 }} /></label>
            <label title={tStatus('層數（Z 方向，1=平面陣列）', lang)}>{tStatus('Z層數', lang)} <input type="number" min={1} step={1} value={featDlg.params.countZ ?? 1} onChange={(e) => setFeatParam('countZ', Number(e.target.value))} style={{ width: 46 }} /></label>
            <label title={tStatus('Z 方向層間距', lang)}>{tStatus(featDlg.params.dtype === 'extent' ? 'Z總長' : 'Z間距', lang)} <input type="number" step={5} value={featDlg.params.dz ?? 60} onChange={(e) => setFeatParam('dz', Number(e.target.value))} style={{ width: 56 }} /></label>
            {(useApp.getState().bodyMesh?.parked?.length ?? 0) > 0 && !featDlg.params.target && (
              <label style={{ justifyContent: 'flex-start', gap: 6, fontSize: 12 }} title={tStatus('對象類型 Bodies（Fusion Object Type）：連所有泊車實體一齊陣列（每個泊車體各自鋪柵格副本）。關＝只陣列活動實體。', lang)}>
                <input type="checkbox" checked={!!featDlg.params.bodies} onChange={(e) => setFeatParam('bodies', e.target.checked ? 1 : 0)} /> {tStatus('連泊車體一齊陣列', lang)}
              </label>
            )}
            <div style={{ display: 'flex', gap: 8, fontSize: 12 }} title={tStatus('對稱（Fusion Symmetric direction）：原件居中，副本向兩邊鋪（僅奇數數量生效，偶數退單向）', lang)}>
            ...
            {useApp.getState().caxes.length > 0 && (<>
              <label title={tStatus('方向1（X 間距沿此方向）：用已建構造軸驅動，缺省=世界 X 軸。選中後 X 間距沿該軸鋪（斜向/對角陣列）。', lang)}>{tStatus('方向1', lang)} <select value={String(featDlg.params.dir1 ?? '')} onChange={(e) => setFeatParam('dir1', e.target.value)} style={{ height: 26 }}><option value="">{tStatus('— 世界 X 軸 —', lang)}</option>{useApp.getState().caxes.map((ca, i) => { const LV: Record<string, [number, number, number]> = { X: [1, 0, 0], Y: [0, 1, 0], Z: [0, 0, 1] }; const dv = ca.dirV ?? LV[ca.dir]; return <option key={'D1A' + i} value={dv.join(',')}>{tStatus('構造軸', lang)}{i + 1}（{ca.dirV ? ca.dirV.map((v) => +v.toFixed(1)).join(',') : ca.dir}）</option> })}</select></label>
              <label title={tStatus('方向2（Y 間距沿此方向）：用已建構造軸驅動，缺省=世界 Y 軸。選中後 Y 間距沿該軸鋪。', lang)}>{tStatus('方向2', lang)} <select value={String(featDlg.params.dir2 ?? '')} onChange={(e) => setFeatParam('dir2', e.target.value)} style={{ height: 26 }}><option value="">{tStatus('— 世界 Y 軸 —', lang)}</option>{useApp.getState().caxes.map((ca, i) => { const LV: Record<string, [number, number, number]> = { X: [1, 0, 0], Y: [0, 1, 0], Z: [0, 0, 1] }; const dv = ca.dirV ?? LV[ca.dir]; return <option key={'D2A' + i} value={dv.join(',')}>{tStatus('構造軸', lang)}{i + 1}（{ca.dirV ? ca.dirV.map((v) => +v.toFixed(1)).join(',') : ca.dir}）</option> })}</select></label>
            </>)}
            {/* GM-3DV1 S3：逐实例抑制 + Compute Option */}
            <label title={tStatus('抑制實例（Fusion Suppression）：逗號分隔要跳過嘅副本索引（0=原件，之後按 (i*Y層+j)*Z層+k 順序）。例：2,5', lang)}>{tStatus('抑制實例', lang)} <input type="text" value={String(featDlg.params.suppress ?? '')} onChange={(e) => setFeatParam('suppress', e.target.value)} placeholder="2,5" style={{ width: 84 }} /></label>
            <label title={tStatus('計算選項（Fusion Compute Option）：優化=快、碰撞副本重算；相同=全部同一形狀（快）；調整=逐副本重算（慢，處理碰撞）。直通提示，v1 不改幾何。', lang)}>{tStatus('計算', lang)} <select value={String(featDlg.params.compute ?? 'optimized')} onChange={(e) => setFeatParam('compute', e.target.value)} style={{ height: 26 }}><option value="optimized">{tStatus('優化', lang)}</option><option value="identical">{tStatus('相同', lang)}</option><option value="adjust">{tStatus('調整', lang)}</option></select></label>
          </>)}
          {featDlg.kind === 'cpattern' && (<>
            <label>{tStatus('軸', lang)} <select value={featDlg.params.axis} onChange={(e) => setFeatParam('axis', e.target.value)} style={{ height: 26 }}><option>X</option><option>Y</option><option>Z</option></select></label>
            <label>{tStatus('數量', lang)} <input type="number" min={2} step={1} value={featDlg.params.count} onChange={(e) => setFeatParam('count', Number(e.target.value))} style={{ width: 46 }} /></label>
            ...
            <label title={tStatus('旋轉中心（CAD 坐標，默認原點）。繞偏心軸陣列時設置。', lang)}>{tStatus('中心', lang)} <input type="number" step={5} value={featDlg.params.cx} onChange={(e) => setFeatParam('cx', Number(e.target.value))} style={{ width: 40 }} /><input type="number" step={5} value={featDlg.params.cy} onChange={(e) => setFeatParam('cy', Number(e.target.value))} style={{ width: 40 }} /><input type="number" step={5} value={featDlg.params.cz} onChange={(e) => setFeatParam('cz', Number(e.target.value))} style={{ width: 40 }} /></label>
            {useApp.getState().cpoints.length > 0 && (
              <label title={tStatus('用已建構造點做旋轉中心：選中後自動填中心 cx/cy/cz（Fusion『繞此點陣列』）', lang)}>{tStatus('構造點', lang)} <select value="" onChange={(e) => { const i = Number(e.target.value); if (!Number.isInteger(i)) return; const cp = useApp.getState().cpoints[i]; if (!cp) return; setFeatParam('cx', cp[0]); setFeatParam('cy', cp[1]); setFeatParam('cz', cp[2]) }} style={{ height: 26 }}><option value="">{tStatus('— 選構造點 —', lang)}</option>{useApp.getState().cpoints.map((cp, i) => <option key={'CP' + i} value={i}>{tStatus('構造點', lang)}{i + 1}（{cp.map((v) => +v.toFixed(0)).join(',')}）</option>)}</select></label>
            )}
          </>)}
          {featDlg.kind === 'circpattern' && (<>
            ...
            {String(featDlg.params.objectType ?? 'bodies') === 'components' && <span style={{ fontSize: 11, color: cpSelCompCount ? '#2c7' : '#c60' }}>{cpSelCompCount ? `✓ ${tStatus('已選組件', lang)} ×${cpSelCompCount}（${tStatus('共享定義的 occurrence 陣列', lang)}）` : tStatus('← 先喺裝配樹勾選 ☑ 或畫布選中組件', lang)}</span>}
            {featDlg.params.target === 'feature' && <span style={{ fontSize: 11, color: cpSelFeat ? '#2c7' : '#c60' }}>{cpSelFeat ? `✓ ${tStatus('已選特徵', lang)} ×${useApp.getState().selectedFeatures.length || 1}` : msg('ui.clickFeatureFirst', lang)}</span>}
            <label title={tStatus('軸方向預設（寫入下面方向分量；要斜軸直接改分量）', lang)}>{tStatus('軸', lang)} <select value={String(featDlg.params.dx) === '1' ? 'X' : String(featDlg.params.dy) === '1' ? 'Y' : String(featDlg.params.dz) === '1' ? 'Z' : 'C'} onChange={(e) => { const v = e.target.value; if (v === 'X') { setFeatParam('dx', 1); setFeatParam('dy', 0); setFeatParam('dz', 0) } else if (v === 'Y') { setFeatParam('dx', 0); setFeatParam('dy', 1); setFeatParam('dz', 0) } else if (v === 'Z') { setFeatParam('dx', 0); setFeatParam('dy', 0); setFeatParam('dz', 1) } }} style={{ height: 26 }}><option value="Z">Z</option><option value="X">X</option><option value="Y">Y</option><option value="C">{tStatus('自定義', lang)}</option></select></label>
            <button className={'cs-btn pick-slot' + (cpatAxisPickA ? ' active' : '')} title={tStatus('拾軸：點活動實體嘅圓柱面（孔壁/圓軸/凸台側面）— 軸點+方向自動填', lang)} onClick={() => useApp.getState().startCpatAxisPick()}>{tStatus('🎯拾軸', lang)}</button>
            {useApp.getState().caxes.length > 0 && (
              <label title={tStatus('用已建構造軸驅動陣列軸：選中後自動填軸點(origin)+方向(dir，含任意斜向 dirV)', lang)}>{tStatus('構造軸', lang)} <select value="" onChange={(e) => { const i = Number(e.target.value); if (!Number.isInteger(i)) return; const ca = useApp.getState().caxes[i]; if (!ca) return; const LV: Record<string, [number, number, number]> = { X: [1, 0, 0], Y: [0, 1, 0], Z: [0, 0, 1] }; const dv = ca.dirV ?? LV[ca.dir]; setFeatParam('ox', ca.at[0]); setFeatParam('oy', ca.at[1]); setFeatParam('oz', ca.at[2]); setFeatParam('dx', dv[0]); setFeatParam('dy', dv[1]); setFeatParam('dz', dv[2]) }} style={{ height: 26 }}><option value="">{tStatus('— 選構造軸 —', lang)}</option>{useApp.getState().caxes.map((ca, i) => <option key={'CA' + i} value={i}>{tStatus('構造軸', lang)}{i + 1}（{ca.dirV ? ca.dirV.map((v) => +v.toFixed(1)).join(',') : ca.dir} @ {ca.at.map((v) => +v.toFixed(0)).join(',')}）</option>)}</select></label>
            )}
            {useApp.getState().cpoints.length > 0 && (
              <label title={tStatus('用已建構造點做陣列軸點（origin）：選中後自動填軸點 ox/oy/oz（Fusion『繞此點陣列』）', lang)}>{tStatus('構造點', lang)} <select value="" onChange={(e) => { const i = Number(e.target.value); if (!Number.isInteger(i)) return; const cp = useApp.getState().cpoints[i]; if (!cp) return; setFeatParam('ox', cp[0]); setFeatParam('oy', cp[1]); setFeatParam('oz', cp[2]) }} style={{ height: 26 }}><option value="">{tStatus('— 選構造點 —', lang)}</option>{useApp.getState().cpoints.map((cp, i) => <option key={'CP' + i} value={i}>{tStatus('構造點', lang)}{i + 1}（{cp.map((v) => +v.toFixed(0)).join(',')}）</option>)}</select></label>
            )}
            <label title={tStatus('軸經過嘅點（CAD 坐標）— 繞偏心軸陣列時設置', lang)}>{tStatus('軸點', lang)} <input type="number" step={5} value={featDlg.params.ox} onChange={(e) => setFeatParam('ox', Number(e.target.value))} style={{ width: 40 }} /><input type="number" step={5} value={featDlg.params.oy} onChange={(e) => setFeatParam('oy', Number(e.target.value))} style={{ width: 40 }} /><input type="number" step={5} value={featDlg.params.oz} onChange={(e) => setFeatParam('oz', Number(e.target.value))} style={{ width: 40 }} /></label>
            <label title={tStatus('軸方向分量（可以斜軸，例 1,0,1）', lang)}>{tStatus('方向', lang)} <input type="number" step={1} value={featDlg.params.dx} onChange={(e) => setFeatParam('dx', Number(e.target.value))} style={{ width: 36 }} /><input type="number" step={1} value={featDlg.params.dy} onChange={(e) => setFeatParam('dy', Number(e.target.value))} style={{ width: 36 }} /><input type="number" step={1} value={featDlg.params.dz} onChange={(e) => setFeatParam('dz', Number(e.target.value))} style={{ width: 36 }} /></label>
            <label>{tStatus('數量', lang)} <input type="number" min={2} max={400} step={1} value={featDlg.params.count} onChange={(e) => setFeatParam('count', Number(e.target.value))} style={{ width: 46 }} /></label>
            <label title={tStatus('完整 = 360° 均分；指定角度 = 副本鋪滿指定角（端點含）；對稱 = 以原件為中心 ± 對稱分布', lang)}>{tStatus('角度', lang)} <select value={featDlg.params.mode} onChange={(e) => setFeatParam('mode', e.target.value)} style={{ height: 26 }}><option value="full">{tStatus('完整 360°', lang)}</option><option value="angle">{tStatus('指定角度', lang)}</option><option value="sym">{tStatus('對稱', lang)}</option></select></label>
            {featDlg.params.mode !== 'full' && <label>{tStatus('總角', lang)} <input type="number" step={15} value={featDlg.params.totalAngle} onChange={(e) => setFeatParam('totalAngle', Number(e.target.value))} style={{ width: 56 }} />°</label>}
            {/* GM-3DV1 S3：逐实例抑制 + Compute Option（环形：0=原件，1..count-1 按角度顺序） */}
            <label title={tStatus('抑制實例（Fusion Suppression）：逗號分隔要跳過嘅副本索引（0=原件，1..數量-1 按角度順序）。例：2,5', lang)}>{tStatus('抑制實例', lang)} <input type="text" value={String(featDlg.params.suppress ?? '')} onChange={(e) => setFeatParam('suppress', e.target.value)} placeholder="2,5" style={{ width: 84 }} /></label>
            <label title={tStatus('計算選項（Fusion Compute Option）：優化/相同/調整。直通提示，v1 不改幾何。', lang)}>{tStatus('計算', lang)} <select value={String(featDlg.params.compute ?? 'optimized')} onChange={(e) => setFeatParam('compute', e.target.value)} style={{ height: 26 }}><option value="optimized">{tStatus('優化', lang)}</option><option value="identical">{tStatus('相同', lang)}</option><option value="adjust">{tStatus('調整', lang)}</option></select></label>
          </>)}
          {featDlg.kind === 'mirror' && (<>
            {/* P2：Fusion Mirror Plane 手势 — 直接点画布任意平面面做镜像面 */}
            <button className={'cs-btn pick-slot' + (mirrorFacePickA ? ' active' : '')} title={tStatus('拾鏡像面：點實體上任意【平面】（唔使先造構造面）', lang)} onClick={() => useApp.getState().startMirrorFacePick()}>{tStatus('🎯拾面', lang)}</button>
            {String(featDlg.params.plane) === 'PICK' && <span style={{ fontSize: 11, color: '#16a36b' }}>{tStatus('✓ 已拾鏡像面', lang)}（[{String(featDlg.params.pickN)}]）</span>}
            <label title={tStatus('鏡像對象：整個實體 = 成件鏡像合併；所選特徵 = 淨鏡像嗰個特徵嘅增/切區域（對稱孔/凸台）；組件 = 鏡像裝配樹勾選/選中嘅組件，支持偏移、構造面及所拾平面。', lang)}>{tStatus('對象', lang)} <select value={featDlg.params.target ?? 'body'} onChange={(e) => setFeatParam('target', e.target.value)} style={{ height: 26 }}><option value="body">{tStatus('整個實體', lang)}</option><option value="feature">{tStatus('所選特徵', lang)}</option><option value="components">{tStatus('組件', lang)}</option></select></label>
            {featDlg.params.target === 'feature' && <span style={{ fontSize: 11, color: cpSelFeat ? '#2c7' : '#c60' }}>{cpSelFeat ? `✓ ${tStatus('已選特徵', lang)} ×${useApp.getState().selectedFeatures.length || 1}` : msg('ui.clickFeatureFirst', lang)}</span>}
            {featDlg.params.target === 'components' && (() => { const n = useApp.getState().checkedComps.length || (useApp.getState().selectedComponent ? 1 : 0); return <span style={{ fontSize: 11, color: n ? '#2c7' : '#c60' }}>{n ? `✓ ${tStatus('已選組件', lang)} ×${n}（${tStatus('支持偏移、構造面及所拾平面', lang)}）` : tStatus('← 先喺裝配樹勾選 ☑ 或選中組件', lang)}</span> })()}
            <label title={tStatus('鏡像基準面：三個世界面（可加偏移）或任意構造面；組件鏡像同樣支持。', lang)}>{tStatus('鏡像面', lang)} <select value={featDlg.params.plane} onChange={(e) => setFeatParam('plane', e.target.value)} style={{ height: 26 }}>{String(featDlg.params.plane) === 'PICK' && <option value="PICK">🎯 {tStatus('拾取面', lang)}</option>}<option>XY</option><option>XZ</option><option>YZ</option>{useApp.getState().planes.map((pl, i) => pl.arb ? <option key={'P' + i} value={'P' + i}>{tStatus('構造面', lang)}{i + 1}</option> : null)}</select></label>
            ...
            {(featDlg.params.target ?? 'body') === 'body' && (
              <label style={{ justifyContent: 'flex-start', gap: 6, fontSize: 12 }} title={tStatus('操作 New Body（Fusion Mirror Operation）：鏡像副本泊車做獨立實體（唔 fuse 入活動體）。關＝Join 融合成一件。', lang)}>
                <input type="checkbox" checked={!!featDlg.params.mirrorOp} onChange={(e) => setFeatParam('mirrorOp', e.target.checked ? 1 : 0)} /> {tStatus('鏡像出獨立實體', lang)}
              </label>
            )}
          </>)}
          ...
          {featDlg.kind === 'rib' && (() => {
            ...
            return (<>
              ...
              {isWeb && <label style={{ justifyContent: 'flex-start', gap: 6, fontSize: 12 }} title={tStatus('延伸曲线（Fusion Web Extend Curves）：把开放折线端點外延到实体墙面，令交叉/近墙筋网自动闭合', lang)}><input type="checkbox" checked={!!+(featDlg.params.extend || 0)} onChange={(e) => setFeatParam('extend', e.target.checked ? 1 : 0)} /> {tStatus('延伸曲线到墙', lang)}</label>}
            </>)
          })()}
          ...
          {featDlg.kind === 'pathpattern' && (<>
            <label title={tStatus('副本總數（包括原始實體／特徵）；沿路徑按等弧長均勻擺放', lang)}>{tStatus('數量', lang)} <input type="number" min={2} max={100} step={1} value={featDlg.params.count} onChange={(e) => setFeatParam('count', Number(e.target.value))} style={{ width: 56 }} /></label>
            <label title={tStatus('整個實體會複製當前實體；所選特徵會只複製時間軸中已選中嘅孔／凸台等特徵', lang)}>{tStatus('對象', lang)} <select value={String(featDlg.params.target ?? 'body')} onChange={(e) => setFeatParam('target', e.target.value)} style={{ height: 26 }}><option value="body">{tStatus('整個實體', lang)}</option><option value="feature">{tStatus('所選特徵（時間軸）', lang)}</option></select></label>
            <label title={tStatus('相同=每個副本保持原始方向；沿路徑=各副本按局部切線作最短旋轉。急彎頂點使用出段切線。', lang)}>{tStatus('方向', lang)} <select value={String(featDlg.params.orient ?? 'identical')} onChange={(e) => setFeatParam('orient', e.target.value)} style={{ height: 26 }}><option value="identical">{tStatus('相同', lang)}</option><option value="path">{tStatus('沿路徑', lang)}</option></select></label>
            <div style={{ fontSize: 11, color: '#6b7680' }}>{tStatus('路徑由剛才草圖提供；取消會還原草圖。', lang)}</div>
          </>)}
          {featDlg.kind === 'combine' && (<>
            <label title={tStatus('合併=兩體並集 · 切除=目標減工具 · 相交=公共體（真 B-rep）。若切除結果為空且對調後有體積，會自動對調（「新實體」後第二體作活動目標時常見）', lang)}>{tStatus('操作', lang)} <select value={featDlg.params.op ?? 'cut'} onChange={(e) => setFeatParam('op', e.target.value)} style={{ height: 26 }}><option value="fuse">{tStatus('＋合併', lang)}</option><option value="cut">{tStatus('－切除(目標−工具)', lang)}</option><option value="common">{tStatus('∩相交', lang)}</option></select></label>
            <div style={{ fontSize: 11, color: '#6b7680', margin: '2px 0' }}>{tStatus('目標 = 活動實體；勾選工具體（泊車）。挖孔：保持大塊為活動體，用工具體（⬡新實體/新實體泊車）切除。', lang)}</div>
            {!(bodyMesh?.parked?.length) && (
              <div role="status" data-testid="combine-need-newbody" style={{ fontSize: 11, color: '#b42318', margin: '4px 0' }}>
                {tStatus('沒有工具體：請先用「新實體」泊車當前體，再建造第二個實體，然後返回合併/布爾。', lang)}
              </div>
            )}
            {(bodyMesh?.parked ?? []).map((b, i) => (
              <label key={i} style={{ justifyContent: 'flex-start', gap: 6 }}>
                ...
                <span>{b.name || (tStatus('實體', lang) + (i + 1))}</span>
              </label>
            ))}
            {/* S185 Keep Tools（Fusion Combine）：保留工具体做独立泊车体（可复用 / 重复布尔），唔消耗 */}
            <label style={{ justifyContent: 'flex-start', gap: 6, marginTop: 2 }} title={tStatus('保留工具體：運算後工具體仍作為獨立泊車實體保留（可再次布爾/導出）；默認消耗', lang)}>
              <input type="checkbox" checked={+(featDlg.params.keepTools ?? 0) > 0} onChange={(e) => setFeatParam('keepTools', e.target.checked ? 1 : 0)} />
              <span>{tStatus('保留工具體（Keep Tools）', lang)}</span>
            </label>
          </>)}
          ...
          {featDlg.kind === 'scale' && (<>
            <label title={tStatus('均勻缩放比例', lang)}>{tStatus('比例', lang)} <input type="number" min={0.01} step={0.1} value={featDlg.params.factor} onChange={(e) => setFeatParam('factor', Number(e.target.value))} style={{ width: 56 }} /> ×</label>
            ...
            {useApp.getState().cpoints.length > 0 && (
              <label title={tStatus('用已建构造点做缩放基准点', lang)}>{tStatus('構造點', lang)} <select value="" onChange={(e) => { const i = Number(e.target.value); if (!Number.isInteger(i)) return; const cp = useApp.getState().cpoints[i]; if (!cp) return; setFeatParam('px', cp[0]); setFeatParam('py', cp[1]); setFeatParam('pz', cp[2]) }} style={{ height: 26 }}><option value="">{tStatus('— 選構造點 —', lang)}</option>{useApp.getState().cpoints.map((cp, i) => <option key={'SCP' + i} value={i}>{tStatus('構造點', lang)}{i + 1}（{cp.map((v) => +v.toFixed(0)).join(',')}）</option>)}</select></label>
            )}
          </>)}
          ...
          {featDlg.kind === 'extrude-edit' && (() => {
            ...
            return (<>
              ...
              {skId && <button className="tb-btn" title={tStatus('重開草圖編輯輪廓（改完全樹重建）', lang)} onClick={() => { const id = featDlg.editId!; cancelFeatDlg(); useApp.getState().editSketchOf(id) }}>✎ {tStatus('編輯草圖', lang)}</button>}
            </>)
          })()}
          ...
          {featDlg.kind === 'revolve' && (<>
            ...
            {useApp.getState().caxes.length > 0 && (
              <label title={tStatus('用已建构造轴驱动旋转轴：选中后自动填轴点(origin)+方向(dir，含任意斜向 dirV)', lang)}>{tStatus('構造軸', lang)} <select value="" onChange={(e) => { const i = Number(e.target.value); if (!Number.isInteger(i)) return; const ca = useApp.getState().caxes[i]; if (!ca) return; const LV: Record<string, [number, number, number]> = { X: [1, 0, 0], Y: [0, 1, 0], Z: [0, 0, 1] }; const dv = ca.dirV ?? LV[ca.dir]; setFeatParam('ox', ca.at[0]); setFeatParam('oy', ca.at[1]); setFeatParam('oz', ca.at[2]); setFeatParam('dx', dv[0]); setFeatParam('dy', dv[1]); setFeatParam('dz', dv[2]) }} style={{ height: 26 }}><option value="">{tStatus('— 選構造軸 —', lang)}</option>{useApp.getState().caxes.map((ca, i) => <option key={'RCA' + i} value={i}>{tStatus('構造軸', lang)}{i + 1}（{ca.dirV ? ca.dirV.map((v) => +v.toFixed(1)).join(',') : ca.dir} @ {ca.at.map((v) => +v.toFixed(0)).join(',')}）</option>)}</select></label>
            )}
            ...
            {useApp.getState().cpoints.length > 0 && (
              <label title={tStatus('用构造点做旋转轴经过点：选中即填轴点(ox/oy/oz)，再撳 X/Y/Z 设方向', lang)}>{tStatus('中心點', lang)} <select value="" onChange={(e) => { const i = Number(e.target.value); if (!Number.isInteger(i)) return; const cp = useApp.getState().cpoints[i]; if (!cp) return; setFeatParam('ox', cp[0]); setFeatParam('oy', cp[1]); setFeatParam('oz', cp[2]) }} style={{ height: 26 }}><option value="">{tStatus('— 選構造點 —', lang)}</option>{useApp.getState().cpoints.map((cp, i) => <option key={'RCP' + i} value={i}>{tStatus('構造點', lang)}{i + 1}（{cp.map((v) => +v.toFixed(0)).join(',')}）</option>)}</select></label>
            )}
          ...
          {featDlg.kind === 'box' && (<>
            ...
            <label>{tStatus('寬', lang)} <LenInput mm={Number(featDlg.params.w)} onMm={(v) => setFeatParam('w', v)} unit={unit} w={52} min={0.1} /></label>
          ...
          {featDlg.kind === 'wedge' && (<>
            ...
            <label>{tStatus('寬', lang)} <input type="number" step={5} min={1} value={featDlg.params.w} onChange={(e) => setFeatParam('w', Number(e.target.value))} style={{ width: 50 }} /></label>
          ...
          {featDlg.kind === 'rtube' && (<>
            <label title={tStatus('截面寬度 X', lang)}>{tStatus('寬', lang)} <input type="number" step={5} min={2} value={featDlg.params.w} onChange={(e) => setFeatParam('w', Number(e.target.value))} style={{ width: 48 }} /></label>
          ...
          {featDlg.kind === 'profile' && (<>
            ...
            <label title={tStatus('截面寬度 X', lang)}>{tStatus('寬', lang)} <input type="number" step={5} min={2} value={featDlg.params.w} onChange={(e) => setFeatParam('w', Number(e.target.value))} style={{ width: 46 }} /></label>
          ...
          {featDlg.kind === 'rbox' && (<>
            ...
            <label>{tStatus('寬', lang)} <input type="number" step={5} min={2} value={featDlg.params.w} onChange={(e) => setFeatParam('w', Number(e.target.value))} style={{ width: 48 }} /></label>
            <label>{tStatus('高', lang)} <input type="number" step={5} min={1} value={featDlg.params.h} onChange={(e) => setFeatParam('h', Number(e.target.value))} style={{ width: 48 }} /></label>
          ...
          {featDlg.kind === 'sheetmetal' && (<>
            ...
            <label>{tStatus('寬', lang)} <input type="number" step={5} min={1} value={featDlg.params.width} onChange={(e) => setFeatParam('width', Number(e.target.value))} style={{ width: 46 }} /></label>
          ...
          {featDlg.kind === 'cpoint' && (<>
            ...
            <button className="cs-btn" title={tStatus('边端點（Fusion Point at Vertex）：点一条边 → 喺最近端點(顶点)落构造点', lang)} onClick={() => { useApp.getState().cancelFeatDlg(); void useApp.getState().startEdgePointPick('vertex') }}>{tStatus('边端點', lang)}</button>
      ...
      {editingComp && (
        <div style={{ position: 'fixed', top: 96, left: '50%', transform: 'translateX(-50%)', zIndex: 220, background: '#fff8ec', border: '1px solid #d9a64e', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,.18)', padding: '6px 12px', display: 'flex', gap: 10, alignItems: 'center', fontSize: 13 }}>
          ...
          <span style={{ color: '#8a7a55', fontSize: 12 }}>{tStatus('時間軸改參數 / 雙擊重開草圖 / 加特徵', lang)}</span>
      ...
      {mode === 'sketch' && (
        <div className="sketch-display-dock" role="group" aria-label={tStatus('顯示方式', lang)}>
          <button
            ref={skMoreRef}
      ...
      {mode === 'sketch' && !modelingCommandActive && (skBarMin ? (
        ...
        <div ref={skBarDrag.ref} className="sketch-bar" style={{ gap: 6, ...skBarPos }}>
          <span className="sb-grip" onPointerDown={skBarDrag.onPointerDown} onClick={() => skBarDrag.consumeClick()} title={tStatus('拖移工具條', lang)} style={{ cursor: 'grab', touchAction: 'none', userSelect: 'none', color: '#9aa4ad' }}>⠿</span>
        ...
        <div ref={skBarDrag.ref} className="sketch-bar" style={skBarPos}>
          ...
          <span className="sb-title" onPointerDown={skBarDrag.onPointerDown} onClick={() => skBarDrag.consumeClick()} title={tStatus('拖移工具條', lang)} style={{ cursor: 'grab', touchAction: 'none', userSelect: 'none' }}>{tStatus('草圖', lang)}</span>
          ...
          {skMorePop && (
            <div className="panel-menu" style={{ position: 'fixed', left: skMorePos.x, top: skMorePos.y, zIndex: 300, minWidth: 250, maxWidth: 340 }} onClick={(e) => e.stopPropagation()}>
              ...
              {([['pt', '端點/点'], ['mid', '中点'], ['center', '圆心'], ['quad', '象限点'], ['x', '交点'], ['tan', '切点']] as const).map(([k, lbl]) => (
      ...
      {sweepDlgOpen && (
        ...
        >
          ...
          {caxes.length > 0 && (
            <label title={tStatus('路径来源：用你画的折线，或拿一条构造轴当直线脊线（沿轴方向扫一段长度）', lang)}>
              ...
              <span><select value={sweepAxis} onChange={(e) => setSweepAxis(e.target.value)} style={{ width: 120 }}>
                ...
                {caxes.map((_, i) => <option key={i} value={'A' + i}>{tStatus('構造軸', lang)}{i + 1}</option>)}
              </select></span>
            </label>
          )}
          {sweepAxis && (
          ...
          <label title={tStatus('3D 爬升：沿路径按弧长均勻升 Z（楼梯扶手/排水管坡道/缆线槽）。0 = 平面扫掠；非 0 时脊线走平滑 3D 样条', lang)}>
      ...
      {selComp && !editingComp && !modelingCommandActive && mode !== 'sketch' && (
        <div className="sketch-bar">
          ...
          <span className="sb-title">{tStatus('移動/旋轉', lang)} {selComp.name}</span>
          {!compBarMin && (<>
          ...
          {(() => { const r = selComp.rot || [0, 0, 0]; const setR = (i: number, v: number) => { const n: [number, number, number] = [r[0], r[1], r[2]]; n[i] = v; setComponentRot(selComp.id, n) }; return (
            <>
              <label title={tStatus('繞 X 軸旋轉（度）', lang)}>{tStatus('繞X°', lang)} <input type="number" step={15} value={r[0]} onChange={(e) => setR(0, Number(e.target.value) || 0)} style={{ width: 52 }} /></label>
              <label title={tStatus('繞 Y 軸（豎直）旋轉（度）— 转动零件朝向最常用', lang)}>{tStatus('繞Y°', lang)} <input type="number" step={15} value={r[1]} onChange={(e) => setR(1, Number(e.target.value) || 0)} style={{ width: 52 }} /></label>
              <label title={tStatus('繞 Z 軸旋轉（度）', lang)}>{tStatus('繞Z°', lang)} <input type="number" step={15} value={r[2]} onChange={(e) => setR(2, Number(e.target.value) || 0)} style={{ width: 52 }} /></label>
              <button className="sb-tool" title={tStatus('清零旋转', lang)} onClick={() => setComponentRot(selComp.id, [0, 0, 0])}>{tStatus('↺歸零', lang)}</button>
            </>
          ) })()}
      ...
      {measureUniMode && (
        <div className="info-card" style={{ position: 'fixed', top: 150, right: 20, width: 250, padding: '12px 14px', zIndex: 200, fontSize: 13 }}>
          ...
          {/* #174-6：Show Snap Points — 显示活动实体特征边端點/中点捕捉标记（橙点） */}
          <label style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 6, fontSize: 11, cursor: 'pointer' }} title={tStatus('显示可捕捉点（特征边端點/中点）', lang)}>
      ...
      {moveFaceMode && (
        <div className="vp-measure" data-testid="move-face-toolbar" style={{ fontWeight: 600, color: '#1572c4', display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button data-testid="move-face-commit" className="tb-btn" style={{ background: moveFacePicks.length ? '#1572c4' : undefined, color: moveFacePicks.length ? '#fff' : undefined }} disabled={!moveFacePicks.length} title={tStatus('应用移动面（Enter）', lang)} onClick={() => void useApp.getState().commitMoveFace()}>{tStatus('確定', lang)}</button>
        </div>
      )}
      ...
      {delFaceMode2 && (
        <div className="vp-measure" style={{ fontWeight: 600, color: '#1572c4', display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button className="tb-btn" style={{ background: delFacePicks2.length ? '#1572c4' : undefined, color: delFacePicks2.length ? '#fff' : undefined }} disabled={!delFacePicks2.length} title={tStatus('删除所选面', lang)} onClick={() => void useApp.getState().commitDelFace()}>{tStatus('確定', lang)}</button>
        </div>
      )}
      {props && showProps && (
        <div ref={propsDrag.ref} className={'vp-props' + (propsDrag.isDragged ? ' vp-hud-dragged' : '') + (propsHudCollapsed ? ' vp-hud-collapsed' : '')} style={propsDrag.style}>
          ...
          📐 {uLen(props.dx)}×{uLen(props.dy)}×{uLen(props.dz)} {uSuf}<span title={tStatus('包围盒空间对角线长（√(dx²+dy²+dz²)）— 装箱/快递/能否斜放入打印床用', lang)} style={{ color: '#8a97a2' }}> {tStatus('(对角', lang)} {uLen(Math.hypot(props.dx, props.dy, props.dz))})</span>　·　{tStatus('體積', lang)} {fmtVol(props.vol, unit)}　·　{tStatus('表面積', lang)} {fmtArea(props.area, unit)}　·　{tStatus('質心', lang)} ({props.com.map((c) => uLen(c)).join(', ')}) {uSuf}
          　·　{tStatus('質量', lang)} {(() => { const g = (props.vol / 1000) * density; return g >= 1000 ? (g / 1000).toFixed(2) + ' kg' : g.toFixed(1) + ' g' })()}
  ...
  return (
    <div data-testid="comp-bool-pick-panel" style={{ position: 'absolute', top: 64, left: '50%', transform: 'translateX(-50%)', background: '#fff', border: '1px solid #2563eb', borderRadius: 8, padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, boxShadow: '0 2px 10px rgba(0,0,0,.14)', zIndex: 60, maxWidth: 'min(92vw, 420px)' }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span style={{ color: '#526574' }}>{tStatus(`來源「${from?.name ?? pending.from}」— 点下方按钮，或浏览树/视口点另一个零件，或勾选其复选框`, lang)}</span>
    ...
    return (
      <FormPalette title={createTitle}>
        ...
        {createKind === 'pipe' ? <>
          ...
          {!['sphere', 'quadball', 'cylinder'].includes(createKind) && <label style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>{createKind === 'torus' ? dimTube : dimWidth} <input type="number" min={1} value={formW} onChange={(e) => setFormW(Number(e.target.value))} style={{ width: 72 }} /></label>}
          {['box', 'cylinder'].includes(createKind) && <label style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>{dimHeight} <input type="number" min={1} value={formH} onChange={(e) => setFormH(Number(e.target.value))} style={{ width: 72 }} /></label>}
          <label style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>{dimFaces} <span><input type="number" min={1} value={formSegA} onChange={(e) => setFormSegA(Number(e.target.value))} style={{ width: 45 }} /> × <input type="number" min={1} value={formSegB} onChange={(e) => setFormSegB(Number(e.target.value))} style={{ width: 45 }} /></span></label>
        </>}
  ...
  return (
    ...
    >
      ...
      <span style={{ display: 'inline-flex', flexWrap: 'wrap', alignItems: 'center', gap: 4, borderLeft: '1px solid #cde', paddingLeft: 8 }} title={tStatus('镜像/对称：cage 沿该轴最小边界面反射并焊接成对称翻倍笼（先造一半再镜成对称形）。', lang)}>
        <span style={{ color: '#2a6fb0', fontWeight: 600 }}>{tStatus('鏡像', lang)}</span>
