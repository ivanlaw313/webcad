...
const META: Record<string, { icon: string; label: string; param: string; field: string; unit: string; fields?: FieldDef[] }> = {
  ...
  offsetsolid: { icon: 'scale', label: '整體偏移', param: '距离', field: 'distance', unit: 'mm', fields: [{ key: 'distance', label: '距离', unit: 'mm' }] },
  ...
  revolve: { icon: 'revolve', label: '旋轉', param: '角度', field: 'angle', unit: '°', fields: [{ key: 'angle', label: '角度', unit: '°' }, { key: 'axis', label: '绕轴', unit: '', opts: [{ value: 'Y', label: 'Y 轴（默认）' }, { value: 'X', label: 'X 轴' }, { value: 'Z', label: 'Z 轴' }] }, { key: 'op', label: '操作', unit: '', opts: [{ value: 'new', label: '加料' }, { value: 'cut', label: '切割(车槽)' }, { value: 'intersect', label: '相交' }] }] },
  fillet: { icon: 'fillet', label: '圓角', param: '半径', field: 'radius', unit: 'mm' },
  facefillet: { icon: 'fillet', label: '面圓角', param: '半径', field: 'radius', unit: 'mm', fields: [{ key: 'radius', label: '半径', unit: 'mm' }] },
  chamfer: { icon: 'chamfer', label: '倒角', param: '距离', field: 'distance', unit: 'mm' },
  shell: { icon: 'shell', label: '抽殼', param: '壁厚', field: 'thickness', unit: 'mm' },
  pattern: { icon: 'pattern', label: '陣列', param: 'X 数量', field: 'countX', unit: '', fields: [{ key: 'countX', label: 'X数量', unit: '' }, { key: 'countY', label: 'Y数量', unit: '' }, { key: 'countZ', label: 'Z数量', unit: '' }, { key: 'dx', label: 'X间距', unit: 'mm' }, { key: 'dy', label: 'Y间距', unit: 'mm' }, { key: 'dz', label: 'Z间距', unit: 'mm' }] },
  prim: { icon: 'box', label: '原语', param: '尺寸', field: 'a', unit: 'mm', fields: [{ key: 'a', label: '尺寸A', unit: 'mm' }, { key: 'b', label: '尺寸B', unit: 'mm' }, { key: 'c', label: '尺寸C', unit: 'mm' }] },
  mirror: { icon: 'mirror', label: '鏡像', param: '偏移', field: 'offset', unit: 'mm', fields: [{ key: 'plane', label: '对称面', unit: '', opts: [{ value: 'YZ', label: 'YZ（左右）' }, { value: 'XZ', label: 'XZ（前后）' }, { value: 'XY', label: 'XY（上下）' }] }, { key: 'offset', label: '面偏移', unit: 'mm' }] },
  ...
  boundarypatch: { icon: 'surf', label: '边界补面', param: '加厚', field: 'thick', unit: 'mm', fields: [{ key: 'thick', label: '加厚', unit: 'mm' }] },
  surfbridge: { icon: 'surf', label: '桥接面', param: '加厚', field: 'thick', unit: 'mm', fields: [{ key: 'thick', label: '加厚', unit: 'mm' }] },
  surfsew: { icon: 'shell', label: '缝合 Stitch', param: '', field: '', unit: '', fields: [] },
  ...
  surfrevolve: { icon: 'revolve', label: '曲面旋轉', param: '角度', field: 'angle', unit: '°', fields: [{ key: 'angle', label: '角度', unit: '°' }, { key: 'axis', label: '绕轴', unit: '', opts: [{ value: 'Y', label: 'Y 轴（默认）' }, { value: 'X', label: 'X 轴' }, { value: 'Z', label: 'Z 轴' }] }] },
  ...
  untrim: { icon: 'shell', label: '去裁/还原', param: '', field: '', unit: '', fields: [] },
  mergefaces: { icon: 'shell', label: '合并面', param: '', field: '', unit: '', fields: [] },  // S 合并同域邻面 Unify-Same-Domain
  editpoles: { icon: 'editpoles', label: '编辑曲面控制点', param: '', field: '', unit: '', fields: [] },  // S133 NURBS 极点编辑
  sweep: { icon: 'sweep', label: '掃掠', param: '半径', field: 'r', unit: 'mm' },
  coil: { icon: 'coil', label: '螺旋', param: '螺距', field: 'pitch', unit: 'mm', fields: [{ key: 'pitch', label: '螺距', unit: 'mm' }, { key: 'height', label: '高度', unit: 'mm' }, { key: 'radius', label: '半径', unit: 'mm' }, { key: 'r2', label: '顶半径', unit: 'mm', defKey: 'radius' }, { key: 'wireR', label: '丝径', unit: 'mm' }] },  // GM-L2 #61：顶半径 r2（锥形弹簧）恒显，缺省=底半径；老档 r2 absent 保持 absent，改动先烘焙
  thread: { icon: 'thread', label: '螺紋桿', param: '螺距', field: 'pitch', unit: 'mm', fields: [{ key: 'd', label: '直径Ø', unit: 'mm' }, { key: 'pitch', label: '螺距', unit: 'mm' }, { key: 'height', label: '高度', unit: 'mm' }] },
  ithread: { icon: 'hole', label: '內螺紋孔', param: '公称Ø', field: 'd', unit: 'mm', fields: [{ key: 'd', label: '公称Ø', unit: 'mm' }, { key: 'pitch', label: '螺距', unit: 'mm' }, { key: 'height', label: '深度', unit: 'mm' }, { key: 'x', label: '中心X', unit: 'mm' }, { key: 'y', label: '中心Y', unit: 'mm' }] },
  cylpatch: { icon: 'cylpatch', label: '曲面貼花', param: '深度', field: 'depth', unit: 'mm', fields: [{ key: 'ang', label: '角度位置', unit: '°' }, { key: 'arc', label: '角宽', unit: '°' }, { key: 'zc', label: '轴向中心', unit: 'mm' }, { key: 'h', label: '轴向高', unit: 'mm' }, { key: 'depth', label: '深度', unit: 'mm' }, { key: 'mode', label: '模式', unit: '', opts: [{ value: 'boss', label: '凸台' }, { value: 'pocket', label: '凹槽' }, { value: 'flat', label: '锉平面' }] }] },
  sheetmetal: { icon: 'sheetmetal', label: '鈑金件', param: '厚度', field: 'thickness', unit: 'mm', fields: [{ key: 'thickness', label: '厚度', unit: 'mm' }, { key: 'radius', label: '折弯半径', unit: 'mm' }, { key: 'kfactor', label: 'K因子', unit: '' }, { key: 'width', label: '宽度', unit: 'mm' }, { key: 'flat', label: '状态', unit: '', opts: [{ value: '0', label: '折叠' }, { value: '1', label: '展开' }] }] },
  pathpattern: { icon: 'pathpattern', label: '路徑陣列', param: '数量', field: 'count', unit: '', fields: [{ key: 'count', label: '数量', unit: '' }] },
  featpattern: { icon: 'pattern', label: '阵列（组）', param: 'X 数量', field: 'cols', unit: '', fields: [{ key: 'cols', label: 'X数量', unit: '' }, { key: 'rows', label: 'Y数量', unit: '' }, { key: 'dx', label: 'X间距', unit: 'mm' }, { key: 'dy', label: 'Y间距', unit: 'mm' }] },  // 特征级组阵列：一个节点含 N 个副本，改行列数即重建
  extgroup: { icon: 'extrude', label: '拉伸组', param: '高度', field: 'height', unit: 'mm', fields: [{ key: 'height', label: '高度', unit: 'mm' }] },  // 多轮廓一次拉伸：一个节点含 N 个轮廓，改高度即全部一齐变（subs 各自保留 操作/贯通）
  gear: { icon: 'gear', label: '齒輪', param: '模数', field: 'module', unit: 'mm', fields: [{ key: 'module', label: '模数', unit: 'mm' }, { key: 'teeth', label: '齿数', unit: '' }, { key: 'thickness', label: '厚度', unit: 'mm' }, { key: 'bore', label: '中心孔Ø', unit: 'mm' }, { key: 'helix', label: '螺旋角β', unit: '°' }] },
  worm: { icon: 'worm', label: '蝸桿', param: '模数', field: 'module', unit: 'mm', fields: [{ key: 'module', label: '模数', unit: 'mm' }, { key: 'starts', label: '头数', unit: '' }, { key: 'length', label: '长度', unit: 'mm' }] },  // T770
  crowngear: { icon: 'crowngear', label: '冠齒輪', param: '模数', field: 'module', unit: 'mm', fields: [{ key: 'module', label: '模数', unit: 'mm' }, { key: 'teeth', label: '齿数', unit: '' }, { key: 'discH', label: '盘厚', unit: 'mm' }, { key: 'faceW', label: '齿宽', unit: 'mm' }, { key: 'bore', label: '孔Ø', unit: 'mm' }] },  // T770
  rack: { icon: 'rack', label: '齒條', param: '模数', field: 'module', unit: 'mm', fields: [{ key: 'module', label: '模数', unit: 'mm' }, { key: 'length', label: '长度', unit: 'mm' }, { key: 'baseH', label: '底座高', unit: 'mm' }, { key: 'thickness', label: '厚度', unit: 'mm' }] },
  pulley: { icon: 'pulley', label: 'V帶輪', param: '外径', field: 'diameter', unit: 'mm', fields: [{ key: 'diameter', label: '外径', unit: 'mm' }, { key: 'width', label: '宽度', unit: 'mm' }, { key: 'bore', label: '中心孔Ø', unit: 'mm' }] },
  scale: { icon: 'scale', label: '缩放', param: '比例', field: 'factor', unit: '×', fields: [{ key: 'factor', label: '等比', unit: '×' }, { key: 'sx', label: '非等比X', unit: '×' }, { key: 'sy', label: '非等比Y', unit: '×' }, { key: 'sz', label: '非等比Z', unit: '×' }, { key: 'px', label: '基准X', unit: 'mm' }, { key: 'py', label: '基准Y', unit: 'mm' }, { key: 'pz', label: '基准Z', unit: 'mm' }] },  // T762
  draft: { icon: 'draft', label: '拔模', param: '角度', field: 'angle', unit: '°' },
  cpattern: { icon: 'cpattern', label: '環形陣列', param: '数量', field: 'count', unit: '', fields: [{ key: 'count', label: '数量', unit: '' }, { key: 'angle', label: '总角度', unit: '°' }, { key: 'axis', label: '绕轴', unit: '', opts: [{ value: 'Z', label: 'Z 轴(水平面内)' }, { value: 'X', label: 'X 轴' }, { value: 'Y', label: 'Y 轴' }] }] },
  ...
  thickenquilt: { icon: 'thicken', label: '加厚整张曲面', param: '板厚', field: 'thick', unit: 'mm', fields: [{ key: 'thick', label: '板厚', unit: 'mm' }] },  // S182
  extendface: { icon: 'extrude', label: '曲面延伸', param: '长度', field: 'ext', unit: 'mm', fields: [{ key: 'ext', label: '长度', unit: 'mm' }] },
  splitface: { icon: 'splitface', label: '分割面', param: '', field: '', unit: '', fields: [] },
  replaceface: { icon: 'replaceface', label: '替换面', param: '', field: '', unit: '', fields: [] },
  moveface: { icon: 'replaceface', label: '移動面', param: '距离', field: 'dist', unit: 'mm', fields: [{ key: 'mode', label: '模式', unit: '', opts: [{ value: 'offset', label: '偏移' }, { value: 'tilt', label: '倾斜' }] }, { key: 'dist', label: '距离', unit: 'mm' }, { key: 'angle', label: '倾斜角', unit: '°' }] },  // GM-B2：移动面 — 距离(offset)/倾斜角(tilt) 可改
  rib: { icon: 'rib', label: '加強筋', param: '厚度', field: 'thickness', unit: 'mm', fields: [{ key: 'thickness', label: '厚度', unit: 'mm' }, { key: 'height', label: '高度', unit: 'mm' }, { key: 'baseZ', label: '基准Z', unit: 'mm' }, { key: 'draft', label: '拔模', unit: '°' }] },
  text: { icon: 'text', label: '文字', param: '字号', field: 'size', unit: 'mm', fields: [{ key: 'size', label: '字号', unit: 'mm' }, { key: 'height', label: '高度', unit: 'mm' }, { key: 'baseZ', label: '基准Z', unit: 'mm' }, { key: 'op', label: '操作', unit: '', opts: [{ value: 'new', label: '加料' }, { value: 'cut', label: '切割' }] }, { key: 'plane', label: '面', unit: '', opts: [{ value: 'XY', label: '上 XY' }, { value: 'XZ', label: '前 XZ' }, { value: 'YZ', label: '右 YZ' }] }] },
  newbody: { icon: 'newbody', label: '新實體', param: '', field: '', unit: '' },
  stepbody: { icon: 'insert', label: 'STEP实体', param: '', field: '', unit: '', fields: [{ key: 'op', label: '操作', unit: '', opts: [{ value: 'new', label: '加料' }, { value: 'cut', label: '切割' }] }] },
  bodyboolean: { icon: 'bodyboolean', label: '實體布爾', param: '目标#', field: 'target', unit: '', fields: [{ key: 'bop', label: '操作', unit: '', opts: [{ value: 'fuse', label: '合并' }, { value: 'cut', label: '切除' }, { value: 'common', label: '相交' }] }, { key: 'target', label: '目标实体#', unit: '' }] },
  split: { icon: 'default', label: '分割', param: '位置', field: 'offset', unit: 'mm', fields: [{ key: 'offset', label: '切割位置', unit: 'mm' }, { key: 'axis', label: '切割轴', unit: '', opts: [{ value: 'X', label: 'X' }, { value: 'Y', label: 'Y' }, { value: 'Z', label: 'Z' }] }, { key: 'keep', label: '保留为活动体', unit: '', opts: [{ value: 'lo', label: '低侧' }, { value: 'hi', label: '高侧' }] }] },  // S128：参数化分割（保历史）
  sketch: { icon: 'sketch', label: '草图', param: '', field: '', unit: '' },  // T756：独立草图（无实体输出 — 双击重开编辑）
  datum: { icon: 'mirror', label: '参考面', param: '偏移', field: 'offset', unit: 'mm', fields: [{ key: 'offset', label: '偏移', unit: 'mm' }, { key: 'angle', label: '角度', unit: '°' }] },  // GM-W5 5.1：参考面/datum = 零几何时间轴节点（改 offset/angle → editFeature → 派生面移位）；angle 只对角度面有实义（其余无害）
  circPattern: { icon: 'cpattern', label: '環形陣列', param: '数量', field: 'count', unit: '', fields: [{ key: 'count', label: '数量', unit: '' }, { key: 'totalAngle', label: '总角度', unit: '°' }, { key: 'mode', label: '模式', unit: '', opts: [{ value: 'full', label: '完整 360°' }, { value: 'angle', label: '指定角度' }, { value: 'sym', label: '对称' }] }] },  // T757
  meshbody: { icon: 'insert', label: '网格实体', param: '', field: '', unit: '' },  // T767：网格→B-rep
  othread: { icon: 'thread', label: '面外螺紋', param: '螺距', field: 'pitch', unit: 'mm', fields: [{ key: 'd', label: '直径Ø', unit: 'mm' }, { key: 'pitch', label: '螺距', unit: 'mm' }, { key: 'height', label: '高度', unit: 'mm' }, { key: 'z0', label: '起点Z', unit: 'mm' }] },  // T775
}
