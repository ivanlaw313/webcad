...
import { buildMoveCandidate, solveMoveCandidate } from './moveRelations'
...
/** One-way source/base Move, followed by regeneration. Generated instances cannot
 ...
 */
export async function movePatternSources(input:PatternDocument,selectedIndices:number[],transform:CopyTransform,options?:PatternDimensionOptions):Promise<(Extract<Awaited<ReturnType<typeof solvePatternCandidate>>,{ok:true}>&{changed:boolean})|{ok:false;reason:string}>{
 const fail=(reason:string)=>({ok:false as const,reason})
 if(input.patterns.length!==1)return fail('Move requires one persistent pattern document.')
 ...
 if(!selectedIndices.length||new Set(selectedIndices).size!==selectedIndices.length||selectedIndices.some(i=>!Number.isInteger(i)||!input.shapes[i]))return fail('Move requires valid source/base geometry selection.')
 if(selectedIndices.some(i=>generated.has(input.entityIds[i])))return fail('Generated instances are managed by the pattern; move the source or detach the pattern first.')
 ...
 const candidate=buildMoveCandidate(baseShapes,baseCons,selected,transform)
 if(!candidate.ok)return candidate
 const solved=await solveMoveCandidate(baseShapes,candidate)
 if(!solved.ok)return solved
 // Disconnected, unselected components are not part of this move transaction.
 ...
 if(baseShapes.some((s,i)=>!touched.has(i)&&solved.shapes[i]!==s))return fail('Move would change unrelated geometry.')
 ...
 // A rotated rectangle can add frame constraints; retain those new solver relations.
