...
for(const intersect of [false,true]) for(const join of ['GeomAbs_Arc','GeomAbs_Intersection']) {
 try {const list=new oc.TopTools_ListOfShape_1();for(const f of removed)list.Append_1(f.wrapped);const b=new oc.BRepOffsetAPI_MakeThickSolid(),progress=new oc.Message_ProgressRange_1();b.MakeThickSolidByJoin(base.wrapped,list,-1,1e-3,oc.BRepOffset_Mode.BRepOffset_Skin,intersect,false,oc.GeomAbs_JoinType[join],false,progress);const {cast}=await import('replicad');const result=cast(b.Shape());const c=new oc.BRepCheck_Analyzer(result.wrapped,true,false);console.log('OPTION',{intersect,join,valid:c.IsValid_2(),volume:measureVolume(result)});c.delete();}catch(e){console.log('ERROR',{intersect,join,error:String(e)})}
}
