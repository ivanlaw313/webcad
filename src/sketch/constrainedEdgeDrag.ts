...
export async function solveConstrainedEdgeDrag(shapes:FShape[],cons:SkCon[],ref:SkRef,intendedEnds:[FPt,FPt]):Promise<ConstrainedEdgeDragResult>{
 ...
 if(!intendedEnds.every(p=>p.every(Number.isFinite)))return{ok:false,reason:'拖动目标无效'}
 if(cons.some(c=>[c.a,c.b,'c'in c?c.c:undefined].some(r=>r?.kind==='edge'&&!validConstraintEdge(shapes,r))))return{ok:false,reason:'约束引用的线段已不存在'}
 ...
 if(!near(delta,secondDelta))return{ok:false,reason:'整条边拖动必须保持长度和方向'}
 ...
 if(straight&&ref.kind==='edge'){
  ...
  for(let i=0;i<2;i++){const a=anchors.point(refs[i]);for(const axis of [0,1] as const){const value=axis===0?a.x:a.y;if(value===undefined)continue;const d=value-old[i][axis];if(fixedDelta[axis]!==undefined&&Math.abs(fixedDelta[axis]!-d)>1e-6)return{ok:false,reason:'两端约束不允许同一刚性平移'};fixedDelta[axis]=d}}
 ...
 try{result=await solveFree(seeded,structuredClone(cons),{ref,ends:targetEnds})}catch{return{ok:false,reason:'整条边约束求解失败'}}
