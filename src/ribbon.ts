...
const SOLID: Panel[] = [
  {
    ...
    tools: [
      ...
      { id: 'createform', label: '创建造型', icon: 'box', quick: true, tip: '切换到 Fusion 式 FORM 细分建模环境；完成后用「完成造型」返回实体。' },
      ...
      { id: 'revolve', label: '旋转', icon: 'revolve', quick: true, tip: '旋转：草图轮廓绕一条轴旋转成回转体（轴/盘/车削件）。可加料/切割车槽/相交，薄壁>0 做灯罩/碗壳。' },
      { id: 'sweep', label: '扫掠', icon: 'sweep', quick: true, tip: '扫掠：一个截面沿你画嘅路径扫出实体（管/导轨）。先画折线/样条路径，壁厚>0 出空心管。' },
      { id: 'loft', label: '放样', icon: 'loft', quick: true, tip: '放样：在两个或多个截面之间平滑过渡成实体。截面可来自 XY/XZ/YZ 或任意参考平面；按选择次序加入。' },
      ...
      {
        id: 'pattern', label: '阵列', icon: 'pattern', quick: true, sep: true, tip: '阵列：矩形 / 环形 / 沿路径 复制实体或孔。',
        children: [
          { id: 'pattern', label: '矩形阵列', icon: 'pattern', tip: '沿 X/Y(/Z) 方向复制实体或孔成网格。' },
          { id: 'circpattern', label: '环形阵列', icon: 'pattern', tip: '绕任意轴等角度复制（Fusion 同款）：对象 = 整个实体或时间轴所选特征（螺栓孔圈/辐条），角度 = 完整360°/指定/对称。' },
          { id: 'geopattern', label: '几何阵列', icon: 'pattern', tip: '复制所选时间轴特征，并为每个实例指定平移或旋转变换。' },
          { id: 'pathpattern', label: '路径阵列', icon: 'pattern', tip: '先建实体→画折线/样条路径→沿路径等距复制实体。' },
        ],
      },
      { id: 'mirror', label: '镜像', icon: 'mirror', quick: true, tip: '镜像特征/实体：跨基准面（XY/XZ/YZ 或所选面）对称复制选中嘅特征或整个实体。同「阵列」配对嘅常用建模操作。' },
      ...
      { id: 'jointorigin', label: '关节原点', icon: 'joint', sep: true, tip: 'Fusion Joint Origin：点实体面心、圆柱孔心或顶点建立可复用命名关节坐标帧。' },
    ],
  },
  {
    name: 'MODIFY',
    tools: [
      { id: 'presspull', label: '按拉', icon: 'presspull', shortcut: 'Q', quick: true, tip: '按/拉：点实体一个平面，沿法向推出或压入（快速加/减料）。' },
      { id: 'editface', label: '编辑面', icon: 'move', tip: 'Fusion Edit Face：选择一个或多个平面面，直接偏移或倾斜并重解相邻面。' },
      { id: 'fillet', label: '圆角', icon: 'fillet', shortcut: 'F', quick: true, sep: true, tip: '圆角：点选棱（可多条）倒成圆弧，设半径（末端半径≠起始=变半径圆角）。', quickChildren: [
        { id: 'fillet', label: '圆角', icon: 'fillet', shortcut: 'F', tip: '圆角：点选棱（可多条）倒成圆弧。' },
        { id: 'chamfer', label: '倒角', icon: 'chamfer', shortcut: 'C', tip: '倒角：等距 / 两距离 / 距离+角度。' },
      ] },
      { id: 'chamfer', label: '倒角', icon: 'chamfer', shortcut: 'C', tip: '倒角（Chamfer）：点选棱切斜角。支持等距 / 两距离 / 距离+角度。' },
      { id: 'shell', label: '抽壳', icon: 'shell', quick: true, sep: true, tip: '抽壳：把实体掏空成等壁厚壳，选开口面。建议喺圆角之前做。' },
      { id: 'draft', label: '拔模', icon: 'draft', tip: '拔模：给侧面加脱模斜度（注塑/铸造件必备）。' },
      { id: 'scale', label: '缩放', icon: 'scale', tip: '缩放：按比例放大/缩小实体。' },
      { id: 'combine', label: '合并/切割', icon: 'combine', quick: true, tip: '合并/布尔：把 活动实体(目标) 同 泊车实体(工具) 做 并起 / 切走 / 相交 —— 出真精确实体（对话框选操作 + 勾要用嘅工具体）。要先用「新实体」整多过一个实体先做到。' },
      { id: 'offsetface', label: '偏移面', icon: 'replaceface', tip: 'Fusion Offset Face：点实体一个或多个平面，沿法向原地偏移；可选择修改现有特征、新偏移或自动。' },
      { id: 'replaceface', label: '替换面', icon: 'replaceface', tip: 'Fusion Replace Face：选择来源面及目标面／平面，以目标几何延伸或裁切实体。当前精确支持平面目标。' },
      { id: 'splitface', label: '分割面', icon: 'splitface', tip: 'Fusion Split Face：用平面或曲面工具分割所选面，实体体积保持不变。' },
      { id: 'splitbody', label: '分割实体', icon: 'split', quick: true, tip: '分割实体：用切割平面把一个实体切成两个独立体（参数化）。' },
      { id: 'silhouettesplit', label: '轮廓分割', icon: 'splitface', tip: 'Fusion Silhouette Split：按所选平面或轴的视图方向，由轮廓曲线生成曲面，再分割面、抽壳实体或实体。' },
      { id: 'move', label: '移动/复制', icon: 'move', shortcut: 'M', quick: true, sep: true, tip: 'Fusion Move/Copy：平移、自由移动、旋转或点到点移动实体／组件，并可建立副本。' },
      { id: 'align', label: '对齐', icon: 'combine', tip: '对齐：先点基准面（唔郁嗰件）再点要郁件嘅面 —— 平面贴平 / 圆柱对准同轴。一次性快捷,唔记录配合；想要持久配合用「关节(拣面)」。' },
      { id: 'delete', label: '删除', icon: 'trash', tip: '删除：删除选中的特征或组件。' },
      { id: 'remove', label: '移除', icon: 'trash', tip: 'Fusion Remove：从当前设计移除所选组件实例；外部来源设计仍然保留。' },
      { id: 'arrange', label: '排列', icon: 'pattern', sep: true, tip: 'Fusion Arrange：选择组件、排列平面及包覆面参数，为制造自动排料。' },
      {
        id: 'simplify', label: '简化', icon: 'scale', tip: 'Fusion Simplify：删除小特征／面，或用基本体替换复杂对象。',
        children: [
          { id: 'removefeatures', label: '删除特征', icon: 'trash', tip: '自动识别并删除选定实体上的小孔、圆角、倒角等特征。' },
          { id: 'removefaces', label: '删除面', icon: 'trash', tip: '删除选定面并自动延伸相邻面来治愈实体。' },
          { id: 'replaceprimitives', label: '使用基本体替换', icon: 'box', tip: '用长方体、圆柱体或球体替换所选对象，并抑制原对象。' },
        ],
      },
      { id: 'vollattice', label: '体积晶格', icon: 'pattern', sep: true, tip: 'Fusion Volumetric Lattice：在封闭实体内部建立体积晶格。' },
      { id: 'voltexture', label: '体积纹理', icon: 'appearance', tip: 'Fusion Volumetric Texture：将体积纹理套用到实体。' },
      { id: 'volmodify', label: '修改体积晶格', icon: 'move', tip: 'Fusion Modify Volumetric Lattice：编辑已有体积晶格特征。' },
      { id: 'physicalmaterial', label: '物理材质', icon: 'appearance', sep: true, tip: 'Fusion Physical Material：只设密度及物理属性，影响质量、惯量、FEA 与 BOM；不会改变外观。' },
      { id: 'appearance', label: '外观', icon: 'appearance', shortcut: 'A', tip: 'Fusion Appearance：打开外观材质球面板，修改颜色、金属度、粗糙度及纹理。' },
      { id: 'managematerials', label: '管理材质', icon: 'appearance', tip: '打开具名外观材质库，保存、套用或删除自订材质预设。' },
  ...
  },
  {
    name: 'INSPECT',
    tools: [
      { id: 'measureuni', label: '测量', icon: 'measure', shortcut: 'I', quick: true, tip: 'Fusion Measure：一个命令测量面、边、点及其组合；显示长度、面积、角度、最短距离及 ΔXYZ。' },
      { id: 'interference', label: '干涉检查', icon: 'interference', tip: 'Fusion Interference：先选择参与检查的实体或组件，再按 Compute 计算真实重叠体积。' },
      { id: 'curvcomb', label: '曲率梳分析', icon: 'curvature', sep: true, tip: 'Fusion Curvature Comb Analysis：沿曲面显示曲率梳齿，检查曲率变化及接缝连续性。' },
      { id: 'zebra', label: '斑马纹分析', icon: 'section', tip: 'Fusion Zebra Analysis：以反射条纹检查曲面切向连续及光顺度。' },
      { id: 'draftanalysis', label: '拔模分析', icon: 'draft', tip: 'Fusion Draft Analysis：按脱模方向着色正拔模、倒扣及垂直区域。' },
      { id: 'curvmap', label: '曲率图分析', icon: 'curvature', tip: 'Fusion Curvature Map Analysis：以色谱显示曲面弯曲趋势。' },
      { id: 'accessanalysis', label: '可达性分析', icon: 'draft', tip: 'Fusion Accessibility Analysis：沿指定方向检查被实体自身遮挡、工具或模具不可达的区域。' },
      { id: 'minradius', label: '最小半径分析', icon: 'curvature', tip: 'Fusion Minimum Radius Analysis：找出模型最小局部曲率半径及位置。' },
      { id: 'section', label: '剖切分析', icon: 'section', quick: true, tip: 'Fusion Section Analysis：以可调剖切平面检查实体内部。' },
      { id: 'centerofmass', label: '质心', icon: 'measure', sep: true, tip: 'Fusion Center of Mass：切换显示实体或装配的体积加权质心标记。' },
      { id: 'properties', label: '物理属性', icon: 'measure', tip: 'Fusion Properties：查看所选实体或组件的质量、体积、表面积、重心与材料。' },
      { id: 'meshfacegroups', label: '显示网格面组', icon: 'draft', sep: true, shortcut: 'Shift+F', tip: 'Fusion Display Mesh Face Groups：切换网格面组颜色显示。' },
    ],
  },
  {
    name: 'INSERT',
  ...
  },
  {
    ...
    tools: [
      ...
      { id: 'rigidgroup', label: '刚性组', icon: 'joint', tip: 'Fusion Rigid Group：在浏览器勾选两个或以上组件，将它们锁成一个刚体组；组节点可在浏览器或关节面板抑制、恢复或删除。' },
      { id: 'motionlink', label: '运动连接', icon: 'joint', sep: true, tip: 'Fusion Motion Link：开启关节面板的运动连接区，为两个转动／滑动关节设传动比（齿轮可用负比反向）。' },
      { id: 'enablecontact', label: '启用接触集', icon: 'joint', tip: 'Fusion Enable Contact Sets：切换装配接触求解；启用后驱动关节会在组件发生实体接触前停止。' },
      { id: 'newcontactset', label: '新建接触集', icon: 'joint', tip: 'Fusion New Contact Set：在浏览器勾选刚好两个组件，登记为命名接触对。' },
      { id: 'motionstudy', label: '运动研究', icon: 'joint', sep: true, tip: 'Fusion Motion Study：开启第一个可驱动关节的动力学设置，设质量、刚度、阻尼及初始位置后求解和播放。' },
      { id: 'drivejoints', label: '驱动关节', icon: 'joint', tip: 'Fusion Drive Joints：展开关节面板，以滑杆、限位、停于接触及播放控制驱动装配。' },
    ],
  },
  {
    name: 'SELECT',
...
const LAB: Panel[] = [
  {
    ...
    tools: [
      ...
      { id: 'sweepedge', label: '扫掠(拾边)', icon: 'sweep', tip: 'WebCAD 扩充：直接拾取实体边链作为扫掠路径。' },
  ...
  },
  {
    name: '检查扩展',
    tools: [
      { id: 'measure', label: '两点距离', icon: 'measure' },
      { id: 'measureedge', label: '边长／孔径', icon: 'measure' },
      { id: 'measureface', label: '面积', icon: 'measure' },
      { id: 'measureangle', label: '面夹角', icon: 'measure' },
      { id: 'properties', label: '完整物理属性', icon: 'measure', quick: true, tip: '面积、密度、质量、体积、包围盒、质心及惯量的详细对话框。' },
...
const SURFACE: Panel[] = [
  { name: 'CREATE', tools: [
    ...
    { id: 'surfsweep', label: '曲面扫掠', icon: 'sweep', quick: true, tip: '曲面扫掠：开放截面沿你画的开放路径扫成零厚开放曲面（风道/导流板/管壁皮）。先画开放折线截面 → 完成草图 → 再画一条开放折线路径 → 撳此。出独立曲面体（要实体再「加厚」/「缝合」）。' },
    { id: 'surfrevolve', label: '曲面旋转', icon: 'revolve', quick: true, tip: '曲面旋转：开放截面绕轴旋成零厚旋转曲面（灯罩/喷嘴/花瓶皮/涡轮毂），无需先做实体再壳。先画开放折线截面 → 撳此 → 输角度,轴(Y/X)。出独立曲面体（要实体再「加厚」/「缝合」）。' },
    { id: 'surfpatch', label: '补面 Patch', icon: 'loft', quick: true, tip: '补面 Patch：喺实体表面/空间顺序点 ≥3 个点、圈出一个闭合边界 → 填充成曲面 + 加厚成薄板。补洞 / 加蒙皮 / 驳面时用。' },
    { id: 'surfbridge', label: '桥接面', icon: 'loft', quick: true, tip: '桥接：点选 2 条现有棱（实体 / 泊车曲面）→ 喺两棱之间起一张光滑过渡面,可加厚。想驳通两条边、封个缺口时用。' },
    { id: 'boundarypatch', label: '边界补面', icon: 'loft', quick: true, tip: '边界补面：拾几条【现有棱】（实体/泊车曲面上）填补一个多边洞 / 封口,可勾相切令接边顺滑；出【曲面体】。先有个带缺口嘅实体/曲面 → 撳此 → 逐条点选边界棱（≥2）→ 可勾相切 / 设加厚 → 确定。' },
    ...
    { id: 'thickenquilt', label: '加厚整张曲面', icon: 'shell', quick: true, tip: '加厚整张曲面：拣一张【完整泊车曲面/缝合面】（放样/扫掠/旋转/缝合/补面出嘅）→ 成张一次过加厚成实体（唔似「加厚」净加单一拾取面）。曲面变实体最常用嘅一步。板厚输负值 = 朝另一边。' },
    { id: 'offsetsurf', label: '偏移曲面', icon: 'loft', tip: '偏移曲面：点一个面 → 偏出一张平行嘅新曲面（正 = 外偏 / 负 = 内偏）。两侧：距离用 ±（如 ±5）= 一次出内外两张平行面（关于原面对称）。出独立开放曲面件。' },
    { id: 'reversesurf', label: '翻转曲面', icon: 'shell', tip: '翻转曲面：拣一张泊车曲面 → 掉转佢嘅正反面（法向反向）。当「加厚」/「缝合」加错咗边时用嚟修正方向。纯翻向,唔重建几何。' },
    { id: 'splitface', label: '分割面', icon: 'splitface', tip: '分割面：点一个面 → 喺点击处用一个垂直平面把佢切成两半（体积不变,两块子面可各自单独拣/着色/拔模）。' },
    { id: 'replaceface', label: '替换面', icon: 'replaceface', tip: '替换面（平面顶替）：点一个面 → 输入推入距离 → 把该面沿法向推去一个新位置,邻面自动延伸接返顺。诚实局限：只做平面顶替（唔支持任意曲面替换）。' },
    { id: 'rotateface', label: '旋转面', icon: 'replaceface', tip: '旋转面：点一个【平面】→ 输入铰轴(X/Y/Z) + 角度 → 该面绕住经过点击点嘅铰线倾斜,邻面自动癒合成梯形（实体保留）。改导入件/斜面角度用。仅限平面。' },
    { id: 'surfsew', label: '缝合 Stitch', icon: 'shell', quick: true, tip: '缝合 Stitch：把现有所有曲面/壳片（放样/补面/加厚/偏移出嘅）沿住共用边焊埋成一个壳；若完全封闭 → 自动转成实体（可继续布尔/导出/打印）。' },
    { id: 'surfunstitch', label: '取消缝合', icon: 'shell', tip: '取消缝合：把缝好嘅壳拆返做逐张独立面（缝合嘅相反操作）,方便单独编辑/删除某一张面。' },
    { id: 'surftrim', label: '平面裁剪', icon: 'shell', tip: '平面裁剪：用一个平面把曲面壳裁走一边、留返另一半。①轴对齐：输 XY/XZ/YZ, 偏移, 保留边；②任意平面：输 原点x,y,z, 法向x,y,z[,保留边]（如 0,0,10,1,0,1,+ = 斜平面裁）。单张散面要先「缝合」成壳。' },
    ...
    { id: 'untrim', label: '去裁/还原', icon: 'shell', tip: '去裁 / 还原：点一个泊车曲面 → 丢弃佢嘅裁剪边界、还原返底层曲面嘅完整原始范围（补返被裁走嘅整片,再重新裁）。裁剪嘅相反操作。' },
    { id: 'intersectcurve', label: '相交曲线', icon: 'shell', tip: '相交曲线：把所有体（活动实体 + 泊车曲面/壳）两两求交,沿交线抽出 3D 参考曲线（青色实线,随项目存档）。想量两曲面点接 / 起截面草图 / 接面参考时用。冇相交就唔变；再撳「清相交曲线」清除。' },
    { id: 'clearintersect', label: '清相交曲线', icon: 'shell', tip: '清除全部相交曲线。' },
    { id: 'mergefaces', label: '合并面', icon: 'shell', tip: '合并面 Merge / Unify Same-Domain：点一个泊车曲面/壳 → 把同域（共面 / 共柱）相邻碎面合并成一张面（裁剪 / 缝合 / 导入后清噪、易拾、STEP 更小）。体积不变，纯拓扑合并。曲面分割的逆向清理。' },
    ...
    { id: 'formbox', label: 'Form 盒', icon: 'box', quick: true },      // T793：Form-lite 细分建模（拖控制点捏有机形）
    { id: 'formcyl', label: 'Form 圆柱', icon: 'box', tip: 'Form 圆柱：由一个圆柱控制笼开始 → 拖控制点捏成 瓶/握把/有机柱。周向段数越多,越贴近你输入嘅半径（细分会向内略收）。✔ 完成后烘焙成网格组件。' },
    { id: 'formplane', label: 'Form 平面', icon: 'box', tip: 'Form 平面/薄片：由一块薄盒控制笼开始 → 拖控制点拉出有机曲面/壳片（座椅/翼面/外壳曲面）。✔ 完成后烘焙成网格组件。' },
    { id: 'formsphere', label: 'Form 球', icon: 'box', tip: 'Form 球：由一个球控制笼开始 → 拖控制点捏成 液滴/头形/有机球。段数越多,越贴近你输入嘅半径（细分会向内略收）。✔ 完成后烘焙成网格组件。' },
    { id: 'formtorus', label: 'Form 环面', icon: 'box', tip: 'Form 环面/甜甜圈：由一个环形控制笼开始 → 拖控制点捏成 把手/O 形/扭环。主半径 > 管半径。✔ 完成后烘焙成网格组件。' },
    ...
    { id: 'editpoles', label: '编辑曲面控制点', icon: 'editpoles', quick: true, tip: '编辑曲面控制点：点一个灰显泊车曲面 → 显示佢嘅控制点球 → 拖三轴箭嘴就整体扭曲张曲面。诚实局限：目前只支持单张曲面；跨缝顺滑接驳后续再做。先用「规则曲面/曲面放样」整一张弯曲曲面。' },
    { id: 'extrude', label: '拉伸', icon: 'extrude', quick: true },
    { id: 'revolve', label: '旋转', icon: 'revolve', quick: true },
    { id: 'sweep', label: '扫掠', icon: 'sweep', quick: true },
    { id: 'loft', label: '放样', icon: 'loft', quick: true },
  ...
  { name: 'MODIFY', tools: [
    ...
    { id: 'combine', label: '合并/切割', icon: 'combine', quick: true },
  ] },
...
const SHEET: Panel[] = [
  ...
  { name: 'MODIFY', tools: [
    ...
    { id: 'fillet', label: '折弯圆角', icon: 'fillet', quick: true },
    { id: 'chamfer', label: '倒角', icon: 'chamfer' },
    { id: 'mirror', label: '镜像', icon: 'mirror' },
    { id: 'pattern', label: '阵列', icon: 'pattern' },
  ] },
  g('CONFIGURE'), g('CONSTRUCT'), g('INSPECT'), g('INSERT'), g('ASSEMBLE'),
  { name: 'SELECT', tools: [{ id: 'select', label: '选择', icon: 'select', quick: true, tip: '选择工具：撳零件/面/边就拣中佢（撳空白处 = 唔拣）；撳 Del 键删除所选。' }] },
...
const PLASTIC: Panel[] = [
  ...
  { name: 'MODIFY', tools: [
    { id: 'shell', label: '抽壳', icon: 'shell', quick: true },
    { id: 'draft', label: '拔模', icon: 'draft', quick: true },
    { id: 'fillet', label: '圆角', icon: 'fillet', quick: true },
    { id: 'chamfer', label: '倒角', icon: 'chamfer' },
  ] },
  g('CONFIGURE'), g('CONSTRUCT'), g('INSPECT'), g('INSERT'), g('ASSEMBLE'),
  { name: 'SELECT', tools: [{ id: 'select', label: '选择', icon: 'select', quick: true, tip: '选择工具：撳零件/面/边就拣中佢（撳空白处 = 唔拣）；撳 Del 键删除所选。' }] },
...
export const FORM_PANELS: Panel[] = [
  { name: 'CREATE', tools: [
    { id: 'formsketch', label: '创建草图', icon: 'sketch', quick: true },
    { id: 'formbox', label: '长方体', icon: 'box', quick: true },
    { id: 'formplane', label: '平面', icon: 'plane', quick: true },
    { id: 'formcyl', label: '圆柱', icon: 'cylinder', quick: true },
    { id: 'formsphere', label: '球', icon: 'sphere', quick: true },
    { id: 'formtorus', label: '圆环', icon: 'torus' },
    { id: 'formquadball', label: '四边形球体', icon: 'sphere' },
    { id: 'formpipe', label: '管道', icon: 'pipeicon' },
    { id: 'formface', label: '面', icon: 'plane', sep: true },
    { id: 'formextrude', label: '拉伸', icon: 'extrude', sep: true },
    { id: 'formrevolve', label: '旋转', icon: 'revolve' },
    { id: 'formsweep', label: '扫掠', icon: 'sweep' },
    { id: 'formloft', label: '放样', icon: 'loft' },
  ] },
  { name: 'MODIFY', tools: [
    { id: 'formedit', label: '编辑造型', icon: 'move', quick: true },
    { id: 'formsubdiv', label: '细分', icon: 'param', quick: true, tip: '细分：提高控制笼细分级（1–3）。预览即时圆滑；完成造型时烘焙。' },
    { id: 'formloop', label: '插入边', icon: 'offset', quick: true },
    { id: 'formcrease', label: '折痕', icon: 'chamfer', tip: '折痕：折硬所选 cage 面四边（细分时棱角企硬）。再执行可取消。' },
    { id: 'formbridge', label: '桥接', icon: 'loft', tip: '桥接（FORM T-spline）：尚未支援 — 请用实体/曲面「桥接面」对实体棱。' },
    { id: 'formweld', label: '焊接', icon: 'combine', tip: '焊接（FORM 顶点）：尚未支援。' },
    { id: 'formfillhole', label: '填补孔', icon: 'loft', tip: '填补孔（FORM）：尚未支援。' },
    { id: 'formerasefill', label: '擦除并填充', icon: 'delface', tip: '擦除并填充：尚未支援。' },
  ] },
  { name: 'SYMMETRY', tools: [
    { id: 'formsymmetry', label: '造型对称', icon: 'mirror', quick: true },
    { id: 'formmirror', label: '镜像', icon: 'mirror' },
  ] },
  { name: 'UTILITIES', tools: [
    { id: 'formrepair', label: '修复实体', icon: 'param', quick: true },
  ] },
  { name: 'CONSTRUCT', tools: [
    { id: 'offsetplane', label: '偏移平面', icon: 'plane', quick: true },
  ] },
  { name: 'INSPECT', tools: [
    { id: 'measureuni', label: '测量', icon: 'measure', quick: true },
  ] },
  { name: 'INSERT', tools: [
    { id: 'forminsert', label: '插入网格', icon: 'insert', quick: true },
  ] },
  { name: 'SELECT', tools: [
    { id: 'select', label: '选择', icon: 'select', quick: true },
  ] },
]
...
export const SKETCH_PANELS: Panel[] = [
  {
    name: 'CREATE',
    tools: [
      { id: 'sk_polyline', label: '直线', icon: 'sketch', glyph: '╱', shortcut: 'L', quick: true, tip: '直线/折线：连续点击画多段相连直线,回到起点或撳「闭合」成轮廓。画紧嗰阵撳 A 键 = 喺 直线 / 相切弧 之间切换（可以直线接弧再接直线）。打数字 = 精确长度；方向自动吸 水平/竖直/45°/平行/垂直。靠近圆会吸住圆嘅【象限点（上下左右最远点）】,由上一点拉向圆会吸住【切点】令该段同圆相切。' },
      { id: 'sk_mline', label: '中点线', icon: 'sketch', glyph: '┿', tip: '中点线（Fusion Midpoint Line）：第一点 = 中点，第二点 = 一端 → 由中点向两端对称嘅直线段（做对称基准/中心参照）。打数字 = 全长。' },
      ...
      { id: 'sk_circle', label: '圆', icon: 'cylinder', glyph: '◯', shortcut: 'C', quick: true, tip: '圆心 + 半径。打数字=精确半径。' },
      { id: 'sk_circle2p', label: '两点圆', icon: 'cylinder', glyph: '⊘', tip: '两点圆（直径两端）：点直径嘅两端两点定圆（第二点定直径与方向）。打数字=精确直径 Ø。' },
      { id: 'sk_circle3', label: '三点圆', icon: 'cylinder', glyph: '◓', tip: '三点圆：点三点定圆（外接圆）— 对齐已有特征/孔位方便。' },
      { id: 'sk_circle2t', label: '两切点圆', icon: 'cylinder', glyph: '◑', tip: '两切点圆：点 2 条直线边 + 设半径 → 画一个同两条边都相切嘅圆（自动加相切约束）。你点边嗰一侧 = 圆落嗰一侧。' },
      { id: 'sk_circle3t', label: '三切点圆', icon: 'cylinder', glyph: '◒', tip: '三切点圆：点 3 条直线边 → 画一个同三条边都相切嘅圆（内切圆/旁切圆,位置按你点击嘅一侧自动定）。' },
      { id: 'sk_arc', label: '圆弧', icon: 'revolve', glyph: '⌒', shortcut: 'A', quick: true, tip: '三点圆弧：起点 → 终点 → 弧上一点（闭合成弓形）。' },
      { id: 'sk_arcc', label: '中心点弧', icon: 'revolve', glyph: '◠', tip: '中心点圆弧（Fusion 同款）：圆心 → 起点（定半径）→ 终点（定弧长，取短向）。真圆弧，可标 R/相切。' },
      ...
      { id: 'sk_rect3', label: '三点矩形', icon: 'box', glyph: '◇', tip: '三点矩形（斜矩形）：两点定一条边，第三点定宽 — 画倾斜嘅矩形。' },
      { id: 'sk_polygon', label: '多边形', icon: 'default', glyph: '⬡', quick: true, tip: '正多边形（中心 + 半径）；草图栏可设边数 / 内切外接。' },
      { id: 'sk_spline', label: '样条', icon: 'sweep', glyph: '∿', quick: true, tip: '样条曲线：连续点击一串点 → 画出一条【经过每一个点】嘅平滑曲线。想画顺滑自由曲线时用（拉伸/旋转出真平滑曲面）。' },
      { id: 'sk_bspline', label: 'B样条', icon: 'sweep', glyph: '⟿', quick: true, tip: 'B样条：连续点击一串控制点 → 画出被呢啲点「拉扯」出嚟嘅平滑曲线（唔一定经过控制点,但比样条更顺滑）。想要极顺滑曲线时用（拉伸/旋转出真平滑曲面）。' },
      { id: 'sk_slot', label: '槽', icon: 'default', glyph: '⬭', quick: true, tip: '腰形槽：两端中心 + 槽宽（草图栏设槽宽）。' },
      { id: 'sk_arcslot', label: '圆弧槽', icon: 'default', glyph: '◜', tip: '圆弧槽：弧心 + 一端 + 另一端，沿弧的腰形槽。' },
      { id: 'sk_rrect', label: '圆角矩形', icon: 'box', glyph: '▢', quick: true, tip: '圆角矩形：对角两点 + 圆角半径（草图栏设半径）。' },
      { id: 'sk_ellipse', label: '椭圆', icon: 'cylinder', glyph: '⬯', quick: true, tip: '椭圆：中心 + 半轴。' },
      { id: 'sk_earc', label: '椭圆弧', icon: 'cylinder', glyph: '◡', tip: '椭圆弧：中心 → 长轴端（定 rx+旋转）→ 短轴半径 → 起角 → 终角（工具面板选择顺 / 逆时针）。真椭圆边，闭合成弓形可拉伸。' },
      { id: 'sk_conic', label: '圆锥曲线', icon: 'cylinder', glyph: '⌓', tip: '圆锥曲线：起点 → 终点 → 顶点（两端切线嘅交点）→ 喺底栏调「充满度」(细 = 扁椭圆弧 / 中 = 抛物线 / 大 = 双曲线)。画出真平滑精确边。' },
      { id: 'sk_point', label: '点', icon: 'default', glyph: '·', quick: true, tip: '草图点：单击落一个构造点（可约束/可标尺寸）— 钻孔定位、对称锚点日常。' },
      { id: 'sk_cline', label: '构造线', icon: 'default', glyph: '┊', quick: true, tip: '构造参考线：点位置即落一条 竖直/水平 长虚线（工具面板切方向）。做对中参考、或做镜像嘅【中心轴】。唔参与拉伸。' },
  ...
  },
  {
    name: 'MODIFY',
    tools: [
      { id: 'sk_trim', label: '修剪', icon: 'default', glyph: '✂', shortcut: 'T', quick: true, tip: '修剪（T，Fusion 同款）：点要剪走嗰段 — 剪到同其它几何嘅相交点；冇相交成条删。闭合轮廓剪完变开放路径。' },
      { id: 'sk_extend', label: '延伸', icon: 'default', glyph: '⟶', tip: '延伸：点开放路径嘅端段 — 沿原方向（直线）/原圆（弧）延长到最近相交几何。' },
      { id: 'sk_break', label: '打断', icon: 'default', glyph: '⊟', tip: '打断（Fusion Break）：点曲线上一点 → 一分为二，两段都保留（近相交点会吸到精确交点）。' },
      { id: 'sk_filletc', label: '倒圆角', icon: 'fillet', glyph: 'r⌒', quick: true, tip: '点近一个直角顶点倒嗰个角（半径=草图栏「圆角R」；或底栏「全部角」一次过倒晒）。' },
      { id: 'sk_chamferc', label: '倒斜角', icon: 'chamfer', glyph: 'C∠', quick: true, tip: '倒斜角：撳近一个直角顶点,把嗰个角倒成一条直斜边（回缩量 = 草图栏「圆角R」；或撳底栏「全部角」一次过倒晒）。' },
      { id: 'sk_offset', label: '偏移', icon: 'default', glyph: '⧉', quick: true, tip: '平行偏移当前轮廓（草图栏设距离，正外/负内）。' },
      { id: 'sk_union', label: '合并轮廓', icon: 'combine', glyph: '∪', quick: true, tip: '合并两个重叠轮廓做一个外框：㩒此即入拾取 → 点第一个轮廓 → 点第二个（即合并）。真圆弧保持。（或先用选择工具选好两个再㩒）' },
      { id: 'sk_subtract', label: '剪走轮廓', icon: 'combine', glyph: '∖', tip: '从第一个轮廓剪走第二个（A−B）：㩒此即入拾取 → 点【保留件】 → 点【剪走件】（即运算）。挖穿出环（外框+孔）/横切裂体都得 — 拉伸自动嵌套判孔。' },
      { id: 'sk_mirrory', label: '镜像', icon: 'mirror', glyph: '⇋', quick: true, tip: '镜像（Fusion 式 2 步）：①点中要镜像嘅轮廓（点一个=拣晒成条相连边，可多个）→「✓拣轴线」→ ②点一条现有【直线边/构造线】做镜像轴 → 反射（保留原件）。工具面板仲有「左右轴/上下轴」一键对称。' },
      { id: 'sk_mirrorx', label: '上下镜像', icon: 'mirror', glyph: '⇅', tip: '上下镜像（一键 across X 轴对称当前轮廓）。' },
      { id: 'sk_array', label: '阵列', icon: 'pattern', glyph: '▦', quick: true, tip: '阵列（Fusion 式）：㩒此入阵列模式 → 底栏面板设 矩形(行列+间距) / 环形(数量+角度+中心)，绿虚线实时预览 → 撳「应用阵列」。对【一个】轮廓。' },
      { id: 'sk_move', label: '移动/复制', icon: 'move', glyph: '✥', tip: '移动/复制当前轮廓：dx,dy 平移 + 绕形心旋转，可出 N 份副本（Fusion Move/Copy）。' },
      { id: 'sk_scale', label: '缩放', icon: 'scale', glyph: '⤢', tip: '草图缩放（Fusion Sketch Scale）：选中轮廓（无选择=全部）绕形心乘系数 k（>1 放大 / <1 缩小；真圆弧/椭圆半径同步缩放）。快捷键喺草图内可用（M=移动 / 缩放喺 ribbon）。' },
      { id: 'sk_project', label: '投影几何', icon: 'default', glyph: '⧉', quick: true, tip: '投影几何：把实体嘅边「印」落当前草图做【真草图曲线】(蓝紫参考线) —— 印落嚟嘅线可标注/拉伸/修改,唔再只系参考。想沿住实体轮廓画嘢时用：撳掣后点实体嘅边即可。（要喺有实体嘅面、或穿过实体嘅平面开草图先印到）' },
      { id: 'sk_constr', label: '构造', icon: 'default', glyph: '╳', shortcut: 'X', tip: '构造几何（X）：把选中轮廓（或当前轮廓）转为虚线参考几何 — 可约束可吸附，唔参与拉伸。再按一次转返实线。' },
      { id: 'sk_guide', label: '扫掠导轨', icon: 'sweep', glyph: '⤳', tip: '导轨扫掠（Fusion guide rail）：先画一条折线做【导轨】→ 撳此记低（橙虚线）→ 再画扫掠【路径】→「沿路径扫掠」— 截面沿路径行进时跟住导轨转向。' },
      { id: 'sk_close', label: '闭合', icon: 'default', glyph: '✓', sep: true, quick: true, tip: '把折线/样条回到起点闭合成轮廓。' },
      { id: 'sk_dxf', label: '导出DXF', icon: 'default', glyph: '⤓', tip: '把当前草图轮廓导出为 2D DXF（激光切割 / AutoCAD）。' },
    ],
  },
  {
    ...
    name: 'CONSTRAINTS',
    tools: [
      { id: 'sk_select', label: '选择', icon: 'select', glyph: '↖', quick: true, tip: '选择工具：点 点/边/圆（最多 2 个）,再撳下面约束掣套用关系；撳 Del 键删除所选。' },
      ...
      { id: 'sk_c_v', label: '竖直', icon: 'default', glyph: '┃', quick: true, tip: '竖直：选 1 条边（或 2 个点）→ 变竖直。' },
      { id: 'sk_c_hv', label: '水平/竖直', icon: 'default', glyph: '┼', tip: '水平/竖直（合一，Fusion 单命令）：选一条或多条边 → 按每条边嘅方向自动判定水平定竖直施加（横向→水平 / 纵向→竖直）。' },
      ...
      { id: 'sk_c_mid', label: '中点', icon: 'default', glyph: '⊹', tip: '中点：选 1 个点 + 1 条边/弧 → 点锁到边中点（弧=真弧中点，非弦中点）。' },
      { id: 'sk_c_conc', label: '同心', icon: 'default', glyph: '◎', tip: '同心：选 2 个圆 → 圆心重合。' },
      { id: 'sk_c_coll', label: '共线', icon: 'default', glyph: '≣', tip: '共线：选 2 条边 → 排成同一直线。' },
      { id: 'sk_c_sym', label: '对称', icon: 'default', glyph: '⇆', tip: '对称：选 2 个点 / 2 条边 / 2 个圆·弧 + 1 条边（轴）→ 关于轴对称（圆对称连半径相等）。' },
      { id: 'sk_d_angle', label: '角度', icon: 'default', glyph: '∠', sep: true, tip: '角度尺寸：用选择工具点 2 条边 → 标夹角（度，可点改驱动几何）。' },
      { id: 'sk_autoconstrain', label: '自动约束', icon: 'default', glyph: '✨', tip: 'AutoConstrain（Fusion wand）：对【选中集】（无选择=全部几何）一次推断多约束（重合/平行/垂直/相切/等半径）。绘制时嘅自动推断喺工具面板可开关。' },
      { id: 'sk_uncon', label: '撤约束', icon: 'undo', glyph: '↶', tip: '移除最后一个约束/尺寸并重新求解（冲突时用）。约束徽章亦可逐个点击移除。' },
    ],
  },
  {
    name: 'FINISH',
    tools: [
      ...
      { id: 'sectionprops', label: '截面属性', icon: 'default', glyph: 'Σ', tip: '截面属性（Fusion Section Properties）：对草图封闭轮廓算 面积 / 形心 / 截面惯矩 Ixx·Iyy·Ixy / 主轴 / 周长（梁弯曲/强度输入）。最大轮廓=外形，内含=孔（自动扣除），分离轮廓忽略。再撳关。' },
    ],
  },
]
