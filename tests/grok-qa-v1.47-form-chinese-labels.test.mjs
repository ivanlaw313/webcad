...
test('v1.47: Pipe Create Form Chinese labels (no plane control)', () => {
  ...
  assert.match(viewport, /zh \? '輪廓' : 'Profile'/)
  assert.match(viewport, /zh \? '圓' : 'Circle'/)
  assert.match(viewport, /zh \? '輪廓面數' : 'Profile Faces'/)
  assert.match(viewport, /輸入路徑點後點確定/)
...
test('v1.47: Create Form dimension labels Chinese', () => {
  assert.match(viewport, /const dimDiameter = zh \? '直徑' : 'Diameter'/)
  assert.match(viewport, /const dimHeight = zh \? '高度' : 'Height'/)
  assert.match(viewport, /const dimFaces = zh \? '面數' : 'Faces'/)
  assert.match(viewport, /const dimMajor = zh \? '大徑 Ø' : 'Major Ø'/)
  assert.match(viewport, /zh \? '長度' : 'Length'/)
  assert.match(viewport, /zh \? '寬度' : 'Width'/)
})
