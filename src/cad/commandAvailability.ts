...
export function commandDisabledReason(s: Context, id: string): string | null {
  ...
  if (s.formMode && new Set(['formsketch', 'formextrude', 'formrevolve', 'formsweep', 'formloft', 'formrepair', 'forminsert']).has(id)) return en ? 'This Form command is not supported yet.' : '此 Form 命令尚未支援。'
