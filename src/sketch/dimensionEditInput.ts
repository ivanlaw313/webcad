...
export function parseDimensionEditInput({con,raw,unit,radDia,params,cons,evaluate}:{con:Dim;raw:string;unit:LenUnit;radDia?:{type:'rad'|'dia';flip:boolean};params:Parameter[];cons:SkCon[];evaluate:Evaluator}):{ok:true;patch:Partial<Dim>}|{ok:false;error:string}{
 ...
 if(numeric!==null||!symbolic){
  ...
  if(parsed.value===undefined)return fail('尺寸無法計算；請檢查數值、單位及語法')
 ...
 if(direct){const value=direct.value*scale;if(!valid(value))return fail('參數尺寸數值無效');return {ok:true,patch:{value,param:direct.name,paramId:parameterId(direct),expr:undefined,refs:undefined,driven:undefined}}}
 ...
 try{assertParameterAcyclic(cons.flatMap(c=>c.kind==='dim'&&c.name?[{id:'dimension:'+c.id,name:c.name,value:c.value,expr:c.id===con.id?body:c.expr,refs:c.id===con.id?refs:c.refs}]:[]))}catch{return fail('尺寸循環引用，已拒絕')}
 const rawValue=evaluate(body,vars)
 if(rawValue===null)return fail('公式無法計算或結果無效')
 const value=rawValue*scale
 if(!valid(value))return fail('公式無法計算或結果無效')
