...
test('v1.48: FORM_PANELS CREATE tools Chinese source', () => {
  ...
  assert.match(ribbon, /id: 'formtorus', label: '圓環'/)
  ...
  assert.match(ribbon, /id: 'formsketch', label: '建立草圖'/)
...
test('v1.48: FORM_PANELS MODIFY / SYMMETRY / UTILITIES Chinese', () => {
  ...
  assert.match(ribbon, /id: 'formloop', label: '插入邊'/)
  assert.match(ribbon, /id: 'formcrease', label: '折痕'/)
  assert.match(ribbon, /id: 'formfillhole', label: '填補孔'/)
  assert.match(ribbon, /id: 'formsymmetry', label: '造型對稱'/)
  assert.match(ribbon, /id: 'formrepair', label: '修復實體'/)
  assert.match(ribbon, /id: 'forminsert', label: '插入網格'/)
...
test('v1.48: EN_LABEL + ZH_GROUP for FORM ribbon', () => {
  ...
  assert.match(i18n, /'四邊形球體': 'Quadball'/)
  assert.match(i18n, /'細分': 'Subdivide'/)
  assert.match(i18n, /'造型對稱': 'Symmetry'/)
  assert.match(i18n, /'插入網格': 'Insert Mesh'/)
