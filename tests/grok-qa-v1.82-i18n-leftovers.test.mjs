...
test('v1.82: profile.name L/U/T localized — no SC 槽钢/角铁 in toast', () => {
  ...
  assert.equal(msg('profile.name.L', 'zh-HK'), 'L 角鐵')
  assert.equal(msg('profile.name.U', 'zh-HK'), 'U 槽鋼')
  assert.equal(profileSectionName('L', 'zh-HK'), 'L 角鐵')
...
test('v1.82 pins: BD-8001 TC · JA 寸法/円錐 · Browser/File · sample/tip', () => {
  assert.equal(msg('status.meshDropArmed', 'zh-HK'), '放開以匯入網格（.stl / .obj / .3mf）')
